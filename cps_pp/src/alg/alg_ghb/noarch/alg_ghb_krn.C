...
//  $Header: /home/chulwoo/CPS/repo/CVS/cps_only/cps_pp/src/alg/alg_ghb/noarch/alg_ghb_krn.C,v 1.1.1.1 2003-06-22 13:34:45 mcneile Exp $
//  $Id: alg_ghb_krn.C,v 1.1.1.1 2003-06-22 13:34:45 mcneile Exp $
 ...
 NHITS, SMALL );
  ...
  for(int HIT_CNT=0; HIT_CNT<NHITS; HIT_CNT++)
  {

    {
      ...
      a = SMALL * URG.Rand(); if(a>0)a*=-1; a+=1.0;
      b = URG.Rand();
      c = URG.Rand();
      d = URG.Rand();
  ...
  ** so that it can be multiplied by the origional link. */
  ...
  for(subblock=0; subblock<=1; subblock++)
  {
    ...
    ** generating theta and phi with the apropriate weighting. */
    ...
    cos_theta = URG.Rand();
    Float	my_pi = 3.14159265358979323846;
    phi = URG.Rand()*my_pi;   /* phi e [-pi,pi) */
