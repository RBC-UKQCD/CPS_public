...
/*!\file
  ...
  $Id: RHMC_Fforce.C,v 1.2 2004-05-10 15:26:55 zs Exp $
*/
//--------------------------------------------------------------------
...
void Fasqtad::RHMC_EvolveMomFforce(Matrix *mom, Vector **sol, int degree,
    ...
    size_t size = GJP.VolNodeSites()/2*FsiteSize()*sizeof(Float);
	...
	Vector *X_o = &(X[p][GJP.VolNodeSites()/2]);
	    
	CgArg cg_arg;
	DiracOpAsqtad asqtad(*this, X_o, X[p], &cg_arg, CNV_FRM_NO);
	asqtad.Dslash(X_o, X[p], CHKB_EVEN, DAG_NO);
#ifdef DEBUGGING
	moveMem(X[p], sol[p], size*2);    // debug only
#endif
