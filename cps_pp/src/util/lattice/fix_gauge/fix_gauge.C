...
HyperPlane::HyperPlane (const Matrix * L0, int Ind2Dir[],
                        int HplDim, Matrix * Gauge,
                        Float SmallFloat):hplane_dim (HplDim), gauge (Gauge),
l0 (L0)
{
  cname = "HyperPlane";
  const char *fname = "HyperPlane(M*,i*,i,M*,F)";
  ...
  //-----------------------------------------------------------------------
  {
    ...
    for (int i = 0; i < hplane_dim; i++)
      num_reals *= XXX::Num_Nodes_in_Dir (ind2dir[i]) * node_size[i];
  ...
  //-----------------------------------------------------------------------
  {
    ...
    for (int i = 0; i < hplane_dim; i++) {
      ...
      VRB.Smalloc (cname, fname, "neighbor_link[i]", neighbor_link[i],
                   mem_size);
...
{
  const char *fname = "HyperPlane_Sum(f)";
...
private:
  ...
  friend IFloat *Lattice::GaugeFixCondSatisfaction (Matrix ** gfix_mat,
                                                    FixGaugeType gfix_type,
                                                    Float SmallFloat) const;
};
...
FixHPlane::FixHPlane (const Matrix * L0, int Ind2Dir[],
                      int HplDim, Matrix * Gauge,
                      Float SmallFloat):HyperPlane (L0, Ind2Dir, HplDim,
                                                    Gauge, SmallFloat)
{
  cname = "FixHPlane";
  const char *fname = "FixHPlane(M*,i*,i,M*,F)";
...
// VRB.Func (cname, fname);
  ...
  for (int i = 0; i < hplane_dim; i++) {
    ...
    if (ind2dir[i] == GJP.XiDir ()) {
      ...
      if (GJP.Bc (i) == BND_CND_GPARITY && GJP.NodeCoor (i) == 0
          && index[i] == -1) {
        //CK crosses a G-parity boundary, must complex conjugate the L and G
        mStarDotMStarPlus ((IFloat *) & tmp_m, (IFloat *) & G (i),
                           (IFloat *) & L (i));
      } else {
    ...
    if (GJP.Bc (i) == BND_CND_GPARITY && GJP.NodeCoor (i) == GJP.Nodes (i) - 1
        && index[i] == node_size[i] - 1) {
      ...
      mStarDotMPlus ((IFloat *) & tmp_m, (IFloat *) & G_plus_i,
                     (IFloat *) & tmp_m1);
    } else {
...
//  VRB.Func (cname, fname);
  ...
  //-------------------------------------------------------------------
  {
    Complex a (A[0]), b (A[1]), c (A[COLORS]), d (A[COLORS + 1]);
    ...
    A[2 * COLORS] = e * v00 + f * v10;
    A[2 * COLORS + 1] = e * v01 + f * v11;
  ...
  //-------------------------------------------------------------------
  {
    ...
    Complex v00 (real (A[COLORS + 1]) + real (d),
                 imag (d) - imag (A[COLORS + 1])),
    ...
    tmp_m[2 * COLORS] = tmp_m[COLORS] * v10;
    tmp_m[COLORS] *= v11;
    tmp_m[2 * COLORS + 1] = tmp_m[COLORS + 1] * v10;
    tmp_m[COLORS + 1] *= v11;
    tmp_m[COLORS + 2] = -v01;
    tmp_m[2 * COLORS + 2] = v00;
  }
...
int Lattice::FixGauge (Float SmallFloat, unsigned long MaxIterNum)
{
...

  {
    int loop_num =
      (fix_gauge_kind ==
       FIX_GAUGE_LANDAU) ? 1 : XXX::Node_Size_in_Dir (NormDir);
    ...
    for (int i = 0; i < loop_num; i++) {
      ...
      if (fix_gauge_ptr[i] != NULL) {
        ...
        VRB.Result (cname, fname, "Initial delta=%g\n", hplane.delta ());
        while ((hplane.delta () > hplane.small_enough)
          ...
          for (int j = 0; j < XXX::CheckFreq; j++) {
            if ((iternum % 100) == 0)
              VRB.Result (cname, fname, "delta[%d]=%g\n", iternum,
                          hplane.delta ());
            hplane.iter ();
            iternum++;
          }
...
IFloat *Lattice::GaugeFixCondSatisfaction (Matrix ** gfix_mat,
                                           FixGaugeType gfix_type,
                                           Float SmallFloat) const
{
  const char *fname =
    "GaugeFixCondSatisfaction(Matrix **gfix_mat, FixGaugeType gfix_type, Float SmallFloat)";
  ...
  int loop_num =
    (gfix_type == FIX_GAUGE_LANDAU) ? 1 : XXX::Node_Size_in_Dir (NormDir);
...
{
  const char *fname = "FixGaugeAllocate(FixGaugeType,i,i*)";

  VRB.Result (cname, fname, "called. GaugeKind=%d NHplanes=%d Hplanes=%p\n",
              GaugeKind, NHplanes, Hplanes);
//    exit(-42);
...

  {
    ...
    if (GJP.Gparity () && fix_gauge_kind != FIX_GAUGE_LANDAU)
      GaugePtrLen *= 2;         //CK G-parity for Coulomb gauge
    ...
    if (GJP.Gparity () && fix_gauge_kind == FIX_GAUGE_LANDAU)
      MemBlockLen *= 2;         //CK G-parity for Landau gauge
  }
...
const Matrix *Lattice::FixGaugeMatrix (int const *pos, const int &flavor)
{
  static const char *fname =
    "FixGaugeMatrix(int const* pos, const int &flavor)";
  VRB.Debug (cname, fname, "fix_gauge_kind=%d\n", fix_gauge_kind);
//  exit(-42);
  if (fix_gauge_kind == FIX_GAUGE_LANDAU) {     //stored in CANONICAL ordering
    ...
    if (GJP.Gparity ())
      hplane += flavor * GJP.NodeSites (normdir);
    ...
    int threepos =
      tposvec[0] + tlenvec[0] * (tposvec[1] + tlenvec[1] * tposvec[2]);

    return &fix_gauge_ptr[hplane][threepos];
  } else
    ERR.General (cname, fname, "Unknown gauge fixing");
}
...
{
  static const char *fname =
    "FixGaugeMatrix(const int &site, const int &flavor)";
  VRB.Debug (cname, fname, "fix_gauge_kind=%d\n", cname, fname, fix_gauge_kind);
//  exit(-42);
...
void Lattice::SetFixGaugeMatrix (const Matrix & mat, int const *pos,
                                 const int &flavor)
{
  static const char *fname =
    "FixGaugeMatrix(int const* pos, const int &flavor)";
...
//    return &fix_gauge_ptr[0][off];
    ...
    if (fix_gauge_ptr[hplane] == NULL)
      ERR.General (cname, fname,
                   "gauge fixing not allocated on %d %d %d %d\n", pos[0],
                   pos[1], pos[2], pos[3]);
