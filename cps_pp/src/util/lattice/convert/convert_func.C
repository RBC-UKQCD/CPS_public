...
#include <comms/cbuf.h>
  CPS_START_NAMESPACE typedef struct ConvertArgStruct
{
  unsigned lx;
  unsigned ly;
  unsigned lz;
...
extern "C"
{
  ...
  extern void site2dram (void *src, void *dst, unsigned *link_tbl,
                         unsigned site_size);
    ...
    for (z = 0; z < cap->lz; z++)
      for (y = 0; y < cap->ly; y++)
        for (x = 0; x < cap->lx; x++) {
          ...
          if (t % 2)
            negate_link (link_size, (IFloat *) site);
          if ((t + x) % 2)
            negate_link (link_size, (IFloat *) site + link_size);
          if ((t + x + y) % 2)
            negate_link (link_size, (IFloat *) site + 2 * link_size);
          ...
          if ((x + y) % 2)
            negate_link (link_size, (IFloat *) site + 2 * link_size);
          if ((x + y + z) % 2)
            negate_link (link_size, (IFloat *) site + 3 * link_size);
#endif
...
#if 1
//non-inline but, treadable version
void RunGConverter (CAP cap, unsigned *site_tbl, unsigned *link_tbl)
{
...
//  sync();
  ...
  size_t vol = cap->vol;
  Float *buf = (Float*)smalloc(sizeof(Float)*cap->site_size*vol);

#pragma omp parallel for default(shared)
  for (size_t low = 0; low < vol; low++) {
    size_t current, desired, tmp;
    Float *cram1, *cram2, *cram_tmp;
...
//               VRB.Flow("",fname,"low=%d\n",low);
    ...
    if (desired & 1) 
    {          // test if site needs conversion

      site2cram (cap->start_ptr + cap->site_size * current,
                 cram1, cap->site_size);

        site2dram (cram1, buf +
                   cap->site_size * (desired >> 1), link_tbl, cap->site_size);

      *(site_tbl + low) = desired & 0xFFFFFFFE;
    }
    else
    ERR.General("",fname,"low=%d already done???\n",low);
  }
#pragma omp parallel for default(shared)
  for (size_t low = 0; low < vol*cap->site_size; low++) {
	*(cap->start_ptr+low) = *(buf+low); 
  }
  sfree(buf);

}
#else
//original, non-threadable 
void RunGConverter (CAP cap, unsigned *site_tbl, unsigned *link_tbl)
{
  char *fname = "RunGConverter";
...
//      uint32_t        low,
  ...
  cram1 = cram1_stack;
  cram2 = cram2_stack;
  for (uint32_t low = 0; low < vol; low++) {
#else
//#pragma omp parallel for default(shared)
  for (uint32_t low = 0; low < vol; low++) {
    uint32_t current, desired, tmp;
    Float *cram1, *cram2, *cram_tmp;
    Float cram1_stack[GSIZE], cram2_stack[GSIZE];
    cram1 = cram1_stack;
    cram2 = cram2_stack;
    ...
    if (desired & 1) {          // test if site needs conversion
      ...
      while (low != desired >> 1) {
        ...
        site2cram (cap->start_ptr +
                   cap->site_size * (desired >> 1), cram2, cap->site_size);

        site2dram (cram1, cap->start_ptr +
                   cap->site_size * (desired >> 1), link_tbl, cap->site_size);

        *(site_tbl + (desired >> 1)) = desired & 0xFFFFFFFE;
...
//                                  18*sizeof(IFloat));
      ...
      Matrix *tmp_gauge = (Matrix *)
        fmalloc (cname_none, fname, "tmp_gauge",
                 cap->vol * 4 * sizeof (Matrix));
...
//                      moveMem(cap->start_ptr,tmp_gauge,cap->vol*4*sizeof(Matrix));
  ...
  case G_WILSON_HB:
    ...
    if (new_str_ord == WILSON || new_str_ord == DWF_4D_EOPREC
        || new_str_ord == DWF_4D_EOPREC_EE)
      VRB.Flow (cname_none, fname, "Converting gauge field order: %s",
                "CANONICAL -> WILSON\n");
    else
      VRB.Flow (cname_none, fname,
    ...
    site_sort_tbl = (unsigned *)
      fmalloc (cname_none, fname, "site_sort_tbl", table_size);
...
//-------------------------------------------------------------------------
    ...
    nstacked = 1;
    if (GJP.Gparity ()) {
      if (new_str_ord == G_WILSON_HB)
        ERR.General (cname_none, fname,
                     "G-parity code not written for G_WILSON_HB\n");
      cb_off *= 2;
      nstacked = 2;
    }
...
//-------------------------------------------------------------------------
              ...
              if (new_str_ord == WILSON || new_str_ord == DWF_4D_EOPREC
                ...
                *(site_sort_tbl + offset) =
                  (x - x % 2 + cap->lx * (y + cap->ly * (z + cap->lz * t))
                   + stk * cb_off / 2 + cb_off * ((x + y + z + t) % 2)) | 1;
              else              // G_WILSON_HB
                *(site_sort_tbl + offset) =
    ...
    link_sort_tbl = (unsigned *)
      fmalloc (cname_none, fname,
               "link_sort_tbl", cap->site_size * sizeof (unsigned));
...
//-------------------------------------------------------------------------
    ...
    if (GJP.Gparity ())
      cap->vol *= 2;            //hack to get GConverter to expect correct number of sites for gauge field if gparity
...
void FcanonToWilson (CAP cap, int num_chkbds)
{
  ...
  site_sort_tbl = (unsigned *)
    fmalloc (cname_none, fname, "site_sort_tbl", cap->vol * sizeof (unsigned));
...
//-------------------------------------------------------------------------
  ...
  component_sort_tbl = (unsigned *)
    fmalloc (cname_none, fname,
             "component_sort_tbl", cap->site_size * sizeof (unsigned));
