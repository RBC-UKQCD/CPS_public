...
/*!\file
  ...
  $Id: f_naive.C,v 1.2 2013-05-16 04:14:06 chulwoo Exp $
*/
//--------------------------------------------------------------------
//  CVS keywords
//
//  $Source: /home/chulwoo/CPS/repo/CVS/cps_only/cps_pp/src/util/lattice/f_naive/f_naive.C,v $
//  $State: Exp $
//
//--------------------------------------------------------------------
//------------------------------------------------------------------
...
//------------------------------------------------------------------
FclassType Fnaive::Fclass() const{
  return F_CLASS_NAIVE;
}

int Fnaive::FsiteSize() const
{
...
// The function returns the total number of CG iterations.
//------------------------------------------------------------------
int Fnaive::FmatInv(Vector *f_out, Vector *f_in, 
		     CgArg *cg_arg, 
		     Float *true_res,
		     CnvFrmType cnv_frm,
		     PreserveType prs_f_in)
{
  ...
  DiracOpNaive naive(*this, f_out, f_in, cg_arg, cnv_frm);
  
  iter = naive.MatInv(true_res, prs_f_in);
...
//------------------------------------------------------------------
int Fnaive::FeigSolv(Vector **f_eigenv, Float *lambda,
		      ...
		      CnvFrmType cnv_frm)
{
  ...
  {
    DiracOpNaive naive(*this, (Vector*) 0 , (Vector*) 0, &cg_arg, CNV_FRM_NO);
    iter = naive.RitzEig(f_eigenv, lambda2, valid_eig, eig_arg);
  }
...
//------------------------------------------------------------------
Float Fnaive::SetPhi(Vector *phi, Vector *frm1, Vector *frm2,
  ...
  DiracOpNaive naive(*this, frm1, frm2, &cg_arg, CNV_FRM_NO) ;
  
  if (dag == DAG_YES) naive.MatPcDag(phi, frm1) ;
  else naive.MatPc(phi, frm1) ;
