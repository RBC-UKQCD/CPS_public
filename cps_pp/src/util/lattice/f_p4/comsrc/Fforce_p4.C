...
/*!\file
  ...
  $Id: Fforce_p4.C,v 1.7 2006-04-13 18:20:37 chulwoo Exp $
*/
//--------------------------------------------------------------------
...
#undef DEBUGGING
    ...
    Vector ***Pmu = (Vector***)amalloc(fmalloc, sizeof(Vector), 3, n_sign, N, GJP.VolNodeSites());
    
    Vector ****P3 = (Vector****)amalloc(fmalloc, sizeof(Vector), 4, n_sign, n_sign, N, GJP.VolNodeSites());
    Vector ****P3prime = (Vector****)amalloc(fmalloc, sizeof(Vector), 4, n_sign, n_sign, N, GJP.VolNodeSites());
    ...
    Vector *****P5prime = (Vector*****)amalloc(fmalloc, sizeof(Vector), 5, n_sign, n_sign, n_sign, N, GJP.VolNodeSites());
	...
	// parallel transport Pmu = U_mu X = P_{ -/+ mu} = Pmu[+/-] for the p4 part
	...
	for (int n=m+1; n<m+4; n++){                        // Loop over nu
	    ...
	    // parallel transport Pnu = U_nu X = P_{ -/+ nu} = Pnu[+/-] for the fat link part
	    ...
	    // parallel transport P3 = U_mu Pnu = P_{-/+nu, +/-mu} = P3[+/-][+/-][i] 
	    // needed in the fat link and p4 parts                                     ms  ns
	    // ns is the nu sign index, ms is the mu sign index,
		...
		for(int kk = 0; kk < n_sign; kk++)
		  memcpy(P3prime[kk][jj][i],P3[kk][jj][i],GJP.VolNodeSites()*sizeof(Vector));
	    // end save
	    // for p4 we need to save only +mu directions, 
	    // can gain a factor two in memory                   
	    for(w=0; w<N; w++){
		for(ns=0; ns<n_sign; ns++){
		    force_product_sum(P3[plus][ns][w], Pnu[ns][w],
				      GJP.p4_staple3_coeff(),
				      force[mu[w]]);
		}
	    }
		...
		if(GJP.p4_staple5_coeff()!=0.0){
		    Float c75 = -GJP.p4_staple7_coeff()/GJP.p4_staple5_coeff();
		...
		if(GJP.p4_staple3_coeff()!=0.0){
		    Float c53 = -GJP.p4_staple5_coeff()/GJP.p4_staple3_coeff();
	    ...
	    // parallel transport P5 = U_mu Pnunu = P_{-/+ nu, -/+ nu, mu}=P5[+/-][+/-][0][i] 
	    // for the fat link and p4 parts                                                        ms  ns
	    ...
	    // here we have to save P5 for later use in the p4 part
	    //memcpy(P5prime, P5, sizeof(Vector)*n_sign*n_sign*N*GJP.VolNodeSites());
	    for(int i = 0; i < N; i++)
	      for(int jj = 0; jj < n_sign; jj++)
		for(int kk = 0; kk < n_sign; kk++)
		  memcpy(P5prime[kk][jj][0][i],P5[kk][jj][0][i],GJP.VolNodeSites()*sizeof(Vector));
	    // end save
	    // for p4 part we only need the +mu directions 
	    // can gain factor 2 in memory 

	    //Lepage term

	    if(fabs(GJP.p4_Lepage_coeff()) > 1e-11){
	    ...
	    if(GJP.p4_staple3_coeff()!=0.0){
		Float cl3 = -GJP.p4_Lepage_coeff()/GJP.p4_staple3_coeff();
	 ...
	 //here comes the p4 ( knight ) term
		...
		force_product_sum(P5prime[plus][minus][0][w], X,
				  GJP.p4_knight_coeff(),
				  force[mu[w]]);
		force_product_sum(P5prime[plus][plus][0][w], X,
				  GJP.p4_knight_coeff(),
				  force[mu[w]]);
	      }
	    ...
	    // 5th and 6th terms : F_nu += Pnunu Pmu^\dagger
	    // more precisely : P_{mu, mu} P_{nu}^{\dagger} - P_{mu, mu} P_{-nu}^{\dagger}
	    for(w=0; w<N; w++)
	      {
		force_product_sum(Pnunu[minus][w], Pmu[minus][w],
				  GJP.p4_knight_coeff(),
				  force[nu[w]]);
		force_product_sum(Pnunu[minus][w], Pmu[plus][w],
				  -GJP.p4_knight_coeff(),
				  force[nu[w]]);
	      }
	
    	    // 7th and 8th terms : F_mu += Pmu Pnunu^\dagger
	    // more precisely : P_{mu} P_{nu,nu}^{\dagger} + P_{mu} P_{-nu,-nu}^{\dagger}
	    for(w=0; w<N; w++)
	      {
		force_product_sum(Pmu[minus][w], Pnunu[minus][w],
				  GJP.p4_knight_coeff(),
				  force[mu[w]]);
		force_product_sum(Pmu[minus][w], Pnunu[plus][w],
				  GJP.p4_knight_coeff(),
				  force[mu[w]]);
	   ...
	   // 9th and 10th terms : F_mu += P3 Pmu^\dagger
	   // more precisely : P_{-nu,mu} P_{-mu}^{\dagger} - P_{nu, mu} P_{-mu}^{dagger} 
	    for(w=0; w<N; w++)
	      {
	    ...
	    // 11th and 12th terms : F_mu += Pmu P3^\dagger
	    // more precisely : P_{mu} P_{-nu,-mu}^{\dagger} - P_{mu} P_{nu, -mu}^{\dagger}
	    for(w=0; w<N; w++)
	      {
