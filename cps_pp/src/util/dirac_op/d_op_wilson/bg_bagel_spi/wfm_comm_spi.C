...
// CJ: "Hacks" needed to compile this with SPI headers only. Needs to check
// if this is necessary or not!


#if 1
#define BGPDMA_VPRINT printf
#define  alloc_L1_aligned(A) memalign(32,A)


const int ERR_ALLOC_SHMEM_FAIL = -1;
const int ERR_INTERNAL_INCONSISTENCY = -2;
const int ERR_COMM_TIMEOUT = -3;
...
inline int get_num_cores(){
  return Kernel_ProcessCount();
}

inline int glob_barrier(){
 return GlobBarrier();
}

inline void chip_barrier(){
  NodeBarrier();
}
...
inline void ppc450_Delay(int delay){
}
...
#define SFK_MAX_D 4
#define NumInjFifos 8 // MUST be >=8
...
static int comm_bytes[4];
static int comm_bytes_s[4];
static int comm_bytes_total;
static int comm_bytes_total_s;
/* the nearest neighbours */
static int nn_x[8];
static int nn_y[8];
static int nn_z[8];
static int nn_t[8];
static int inj_fifo_size_in_bytes[NumInjFifos];
static int Pid;
static unsigned char hints[6];
...
//void wfm::spi_init(WilsonArg *wilson_p)
void spi_init()
{
    ...
    unsigned int DIR_X = CPS_NAMESPACE::DIR_X;
    unsigned int DIR_Y = CPS_NAMESPACE::DIR_Y;
    unsigned int DIR_Z = CPS_NAMESPACE::DIR_Z;
    unsigned int DIR_T = CPS_NAMESPACE::DIR_T;
...
//    exit(-42);
    ...
    nodes_t     = get_num_cores();
    if(my_x<2 && my_y<2 && my_z<2&& my_t<2){
      printf("wfm_coor=%d %d %d %d\n",my_x,my_y,my_z,my_t);
      printf("wfm_nodes=%d %d %d %d\n",nodes_x,nodes_y,nodes_z,nodes_t);
    }
    max_x       = (nodes_x - 1);
    max_y       = (nodes_y - 1);
    max_z       = (nodes_z - 1);
    ...
    if ( rc ==0) { 
//	printf(" wfm_bgpdma: SUCCESS for DMA_CounterGroupAllocate DMA_Type_Injection\n"); 
    }
    else { 
    ...
    if ( rc ==0) { 
//	printf(" wfm_bgpdma: SUCCESS for DMA_CounterGroupAllocate DMA_Type_Reception\n"); 
    }
    else { 
    ...
    rc = DMA_InjFifoGroupAllocate(inj_f_grp,                //  injection fifo grp,
				  ...
				  NULL,           // rget_handler
				  NULL,           // rget_handler_parm
				  NULL,           // rget_interruptGroup
				  NULL,           // rget_barrier
				  &inj_fifo_group);    //  return structure
    if ( rc == 0) {
//	printf(" wfm_bgpdma: SUCCESS for DMA_InjFifoGroupAllocate\n");
    }
    else {
...
//    for(mu=0; mu<SFK_MAX_D; mu++) {
	...
	/* Add the number of descriptors needed to comunicate this phase. The 
	   result is the same for the - direction. */
        /* always 1 for BG/(LP) - CJ */
	inj_fifo_num_descriptors[(2*mu)]   = 1;
	inj_fifo_num_descriptors[(2*mu)+1] = 1;
...
//    glob_barrier();
    ...
    /*--------------------------------------------------------------------------
      ...
      --------------------------------------------------------------------------*/
    comm_bytes_total = 0;
    int slx = local_latt[0];
    int sly = local_latt[1];
    int slz = local_latt[2];
    int slt = local_latt[3];
    unsigned long vol = (slx*sly*slz*slt)/2;
	...
	/* If the grid does not end in the + direction of this dimension add 
	   ...
	   for the - direction. */
	comm_bytes[mu] = ( bound* HALF_SPINOR_SIZE* sizeof(double));
        comm_bytes_s[mu] = comm_bytes[mu]/2;
        if (!local_comm[mu])
          comm_bytes_total+=comm_bytes[mu];
    }
    Vprintf("comm_bytes_total=%d\n",comm_bytes_total);
    comm_bytes_total_s = comm_bytes_total/2;
    /* single prec */
	...
	if (rc) {
//	    printf(" wfm_bgpdma: FAILURE for DMA_InjFifoInitById, " "Ififo[%i], rc=%i \n", i, rc);
	    KillJob(rc);
	}
...
//	    printf(" wfm_bgpdma: FAILURE for DMA_InjFifoInitById, " "Ififo[%i], rc=%i \n", i, rc);
	...
	if (Pid==iPid){
		...
		blk_byte_size = comm_bytes[mu];
		Printf("blk_byte_size[%d]=%d\n",mu,comm_bytes[mu]);

		/* initialize send address */
		send_off = (uint32_t)send_bufs[Plus][mu]
		-  (uint32_t)send_bufs[Plus][0];

		/* initialize receive address */
		recv_off = (uint32_t)recv_bufs[Plus][mu]
		- (uint32_t)recv_bufs[Plus][0];
		...
		/* Generate one describtor per consecutive data block */
			...
			if( rc == 0) {
			    Printf(" wfm_bgpdma: SUCCESS for DMA_TorusDirectPutDescriptor torus\n");
...
//	NODE_BARRIER;
	...
	if (Pid==iPid){
		...
		/* initialize send address */
		send_off = (uint32_t)send_bufs[Minus][mu]
		- (uint32_t)send_bufs[Minus][0];

		/* initialize receive address */
		recv_off = (uint32_t)recv_bufs[Minus][mu]
		- (uint32_t)recv_bufs[Minus][0];
...
//	NODE_BARRIER;
	...
	case 0:
	    ...
	    bgp_wfm_shmptr = (char *)mmap( NULL, MAX_SHARED_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
	...
	case 1:
	    ...
	    bgp_wfm_shmptr = (char*)mmap( NULL, MAX_SHARED_SIZE, PROT_READ | PROT_WRITE, 
			    MAP_SHARED, fd, 0);
...
void wfm::spi_fwd_start()
{
    ...
    rc = DMA_CounterSetValueBaseMaxById(&rec_counter_group, 
					...
					comm_bytes_total, 
					recv_bufs[Plus][0],
					recv_bufs[Plus][3]+PAD_HALF_SPINOR_SIZE * nbound[3]);
...
void wfm::spi_bwd_start()
{
    ...
    rc = DMA_CounterSetValueBaseMaxById(&rec_counter_group, 
					...
					comm_bytes_total, 
					recv_bufs[Minus][0],
					recv_bufs[Minus][3]+PAD_HALF_SPINOR_SIZE * nbound[3]);
...
void wfm::spi_comm_wait(int dir)
{
    int pn = dir%2;
    if (pn){
	wfm_bgpdma_bwd_wait();
