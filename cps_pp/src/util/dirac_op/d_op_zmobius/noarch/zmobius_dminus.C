#include<config.h>
CPS_START_NAMESPACE
//--------------------------------------------------------------------
//  CVS keywords
//
//  $Author: chulwoo $
//  $Date: 2013-04-05 17:46:30 $
//  $Header: /home/chulwoo/CPS/repo/CVS/cps_only/cps_pp/src/util/dirac_op/d_op_mobius/noarch/mobius_dminus.C,v 1.2 2013-04-05 17:46:30 chulwoo Exp $
//  $Id: mobius_dminus.C,v 1.2 2013-04-05 17:46:30 chulwoo Exp $
//  $Name: not supported by cvs2svn $
//  $Locker:  $
//  $RCSfile: mobius_dminus.C,v $
//  $Revision: 1.2 $
//  $Source: /home/chulwoo/CPS/repo/CVS/cps_only/cps_pp/src/util/dirac_op/d_op_mobius/noarch/mobius_dminus.C,v $
//  $State: Exp $
//
//--------------------------------------------------------------------
//------------------------------------------------------------------
//
// The in, out fields are defined on the checkerboard lattice
// cb = 0/1 <--> even/odd checkerboard of in field.
// dag = 0/1 <--> Dslash/Dslash^dagger is calculated.
//
//------------------------------------------------------------------

CPS_END_NAMESPACE
#include<config.h>
#include<util/zmobius.h>
#include<util/dwf.h>
#include<util/wilson.h>
#include<util/gjp.h>
#include<util/vector.h>
#include<util/verbose.h>
#include<util/error.h>
#include<comms/scu.h>
CPS_START_NAMESPACE
#ifdef PARALLEL
CPS_END_NAMESPACE
#include <comms/sysfunc_cps.h>
CPS_START_NAMESPACE
#endif


void zmobius_dminus(Vector *out, 
		   Matrix *gauge_field, 
		   Vector *in, 
		   int cb, 
		   int dag, 
		   Zmobus *mobius_lib_arg)
{

  int i;
  int ls;
  IFloat *frm_in;
  IFloat *frm_out;
  IFloat *g_field;
  Wilson *wilson_p;
  int size_cb[2];
  int parity;
  
  //----------------------------------------------------------------
  // Initializations
  //----------------------------------------------------------------
  ls = mobius_lib_arg->ls;
  const size_t f_size = 24 * mobius_lib_arg->vol_4d / 2;


  frm_in = (IFloat *) in;
  frm_out = (IFloat *) out;
  g_field = (IFloat *) gauge_field;
  wilson_p = mobius_lib_arg->wilson_p;
  size_cb[0] = 24*wilson_p->vol[0];
  size_cb[1] = 24*wilson_p->vol[1];
  
  IFloat* frm_;
  Vector  *frm_tmp3 = (Vector *) mobius_lib_arg->frm_tmp3;
  frm_ = (IFloat*)frm_tmp3;

  //----------------------------------------------------------------
  // Apply 4-dimensional Dslash
  //----------------------------------------------------------------

  // frm_ = c * in
#if 0
  Float c_coeff = GJP.Mobius_c();
  vecEqualsVecTimesEquFloat(frm_, frm_in, -0.5*c_coeff, f_size*ls);
#else
  Complex* c_coeff = GJP.ZMobius_c();
  if(dag)
    for(i=0; i<ls; i++){
      vecEqualsVecTimesEquComplex((Complex*)(frm_+i*f_size),
				  (Complex*)(frm_in+i*f_size),
				  -0.5*conj(c_coeff[i]), f_size);
  }
  else
    for(i=0; i<ls; i++){
      vecEqualsVecTimesEquComplex((Complex*)(frm_+i*f_size),
				  (Complex*)(frm_in+i*f_size),
				  -0.5*c_coeff[i], f_size);
  }
#endif
  
  // out = D_W * frm_
  for(i=0; i<ls; i++){

    // parity of 4-D checkerboard
    //------------------------------------------------------------
    parity = cb;//4d odd-even preconditioning

    // Apply on 4-dim "parity" checkerboard part
    //------------------------------------------------------------
    wilson_dslash(frm_out, g_field, frm_, parity, dag, wilson_p);
    
    frm_ = frm_ + size_cb[parity];
    frm_out = frm_out + size_cb[parity];
  }

}



CPS_END_NAMESPACE
