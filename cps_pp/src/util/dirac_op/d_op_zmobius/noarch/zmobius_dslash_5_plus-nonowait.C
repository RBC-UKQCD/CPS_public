			    ...
			    )
{
  ...
  const int vol_4d_cb   = zmobius_lib_arg->vol_4d / 2;
...
void zmobius_kappa_dslash_5_plus_a_five(Vector *out, 
			      Vector *in, 
			      Float mass,
			      int dag, 
			      Zmobus *mobius_lib_arg,
...
{
  if (dag == 0) 
    zmobius_kappa_dslash_5_plus_dag0(out, in, mass, mobius_lib_arg, a_five_inv);
  else
    zmobius_kappa_dslash_5_plus_dag1(out, in, mass, mobius_lib_arg, a_five_inv);
}
		       ...
		       Zmobus *mobius_lib_arg, Float fact)
{
  if (dag == 0) 
    ...
    zmobius_kappa_dslash_5_plus_dag1(out, in, mass, mobius_lib_arg, fact);
}
