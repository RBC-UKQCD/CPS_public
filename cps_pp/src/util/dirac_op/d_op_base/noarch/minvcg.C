...
{
  char *fname = "MInvCG(V*,V**,...)[d_op_base/noarch]";
...
//------------------------------------------------------------------
  ...
  if ((lat.Fclass () != F_CLASS_DWF) && (GJP.Snodes () > 1))
    ERR.General (cname, fname,
                 "Fermion class type inconsistent with spread-out S dimension\n");


// Print out input parameters
//------------------------------------------------------------------
  VRB.Result (cname, fname, "number of shifts = %d\n", Nmass);
  VRB.Result (cname, fname,
              "smallest shift stop_rsd = %e\n", IFloat (RsdCG[0]));
  VRB.Result (cname, fname, "max_num_iter = %d\n", dirac_arg->max_num_iter);
...
//------------------------------------------------------------------
  ...
  Vector **p =
    (Vector **) smalloc (Nmass * sizeof (Vector *), cname, fname, "p");
    ...
    if (type == SINGLE)
      for (int s = 0; s < Nmass; s++) {
        if (convsP[s]) {
          ...
          VRB.Flow (cname, fname, "bs[%d]=%g psi[%d]=%g\n", s, bs[s], s,
                    *tmp_p);
          continue;
        }
  ...
  int Nvec=Nmass;
  if (type == SINGLE) Nvec=1;
  for(int i=0;i<Nvec;i++){
    Float *v_p = (Float*) psi[i]; 
    VRB.Result(cname,fname,"out[%d] = %g %g\n", i,*v_p,*(v_p+1));
  }
