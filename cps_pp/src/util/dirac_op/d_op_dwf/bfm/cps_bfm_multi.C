...
{
  char *fname="MInvCG(V**,V*,F,F*,i,i,F*,t,F*)[bfm DWF version]";
  ...
  TimeStamp::stamp("Starting qdp init");
  cps_qdp_init(GJP.argc_p(), GJP.argv_p());
  TimeStamp::stamp("Finished qdp init");
  int iter=0;

  double minv_time = -dclock();
  TimeStamp::stamp("Starting src norm calc");
  Float src_norm_sq = in->NormSqNode(f_size_cb);
  DiracOpGlbSum(&src_norm_sq);
  TimeStamp::stamp("Finished src norm calc");
...
#undef COMMANDLINE
    ...
    TimeStamp::stamp("Starting bfm init");
    dwf.init(dwfa);
    TimeStamp::stamp("Finished bfm init");
    ...
    TimeStamp::stamp("Starting residual setup");
#define NMULTI (3)
    ...
    Float *gauge = (Float*) lat.GaugeField();
    {
      ...
      TimeStamp::stamp("Start CPS->QDP gauge convert");
      importGauge(lat,U,gauge,1);
      TimeStamp::stamp("Start gauge import");
      dwf.importGauge(U);
      TimeStamp::stamp("Finished gauge import");
    }
      ...
      
    {
      ...
      TimeStamp::stamp("Start CPS->QDP fermion convert");
      impexFermion(0,lat,src_qdp,src_cps,0,1);
      TimeStamp::stamp("Start fermion import");
      dwf.importFermion(src_qdp,src_bfm,1);
      TimeStamp::stamp("Finished fermion import");
    }
    ...
    TimeStamp::stamp("Start multi-shift inverter");
    double bfm_time = -dclock();
    bfm_spawn_cg(dwf);
    bfm_time  += dclock();
    TimeStamp::stamp("Finished multi-shift inverter");
	...
	TimeStamp::stamp("Start fermion export");
	dwf.exportFermion(sol_qdp,sol_bfm[i],1);
	print_vec(sol_bfm[i],"sol_bfm");
	TimeStamp::stamp("Start QDP->CPS fermion convert");
	impexFermion(1,lat,sol_qdp,sol_cps,0,1,0,fac);
	TimeStamp::stamp("Finished QDP->CPS fermion convert");
