...
//  $Header: /home/chulwoo/CPS/repo/CVS/cps_only/cps_pp/src/util/dirac_op/d_op_dwf/qcdsp_nos_hdw_diag/dwf_dslash_5_plus.C,v 1.1.1.1 2003-06-22 13:34:46 mcneile Exp $
//  $Id: dwf_dslash_5_plus.C,v 1.1.1.1 2003-06-22 13:34:46 mcneile Exp $
//  $Name: not supported by cvs2svn $
//  $Locker:  $
//  $Log: not supported by cvs2svn $
//  Revision 1.6  2001/08/16 10:50:19  anj
...
//
//  Revision 1.4  2001/07/03 17:01:02  anj
//
...
//  $Revision: 1.1.1.1 $
//  $Source: /home/chulwoo/CPS/repo/CVS/cps_only/cps_pp/src/util/dirac_op/d_op_dwf/qcdsp_nos_hdw_diag/dwf_dslash_5_plus.C,v $
//  $State: Exp $
//
//--------------------------------------------------------------------
//------------------------------------------------------------------
...
{
  // Get wire numbers
  int wire0 = dwfso_wire_map[0];
  int wire1 = dwfso_wire_map[1];

  // Increment communication call number
  wfm_scu_diag[0] = wfm_scu_diag[0] + 1;

  // Start communications
  *( (IFloat**)(dsp_scu_base0x8+wire1) ) = send_buf;
  *( (IFloat**)(dsp_scu_base0x0+wire0) ) = rcv_buf;

  // Poll wire0 and set wfm_scu_diag entry
  for(int i0=wfm_max_scu_poll; i0>0; i0--){
    if( *(dsp_scu_base0x10+wire0) == 0 ) break;
  }
  wfm_scu_diag[wire0+3] = i0;

  // Poll wire1 and set wfm_scu_diag entry
  for(int i1=wfm_max_scu_poll; i1>0; i1--){
    if( *(dsp_scu_base0x10+wire1) == 0 ) break;
  }
  wfm_scu_diag[wire1+3] = i1;

  // Copy scu status register 813040 into  wfm_scu_diag[2] 
  wfm_scu_diag[2] = *(dsp_scu_base0x40);

  // Copy scu error status register 81306x into  wfm_scu_diag[11+x] 
  wfm_scu_diag[11+wire0] = *(dsp_scu_base0x60+wire0);
  wfm_scu_diag[11+wire1] = *(dsp_scu_base0x60+wire1);

  // If any polling failed call error routine
  if( i0==0 || i1==0 ){
    dwf_scu_error();
  }
...
void getMinusFiveData(IFloat* rcv_buf, IFloat *send_buf)
{
  ...
  // Start communications
  *( (IFloat**)(dsp_scu_base0x8+wire0) ) = send_buf;
  *( (IFloat**)(dsp_scu_base0x0+wire1) ) = rcv_buf;
