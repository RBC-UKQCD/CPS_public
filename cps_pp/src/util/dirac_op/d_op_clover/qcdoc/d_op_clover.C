  ...
  : DiracOpWilsonTypes(latt, f_field_out, f_field_in, arg, cnv_frm_flg)
{
  ...
  int tmp_size = lat.FsiteSize()*GJP.VolNodeSites()/2;  
  clover_tmp =  (Vector *) smalloc(tmp_size * sizeof(Float));
  if (clover_tmp == 0) 
    ERR.Pointer(cname, fname, "clover_tmp");
  VRB.Smalloc(cname,fname, "clover_tmp", clover_tmp, tmp_size);
...
//  VRB.Func(cname,fname);
  ...
  // frm_buf1 = MatPc in
  //--------------------------------------------------------------------
  MatPcDagOrNot(frm_buf1, in, 0, dot_prd );
...
//  VRB.Func(cname,fname);
  ...
  int quarter_sites    = half_sites/2;
...
#endif
#if 1
    wfm_dslash_begin((IFloat *)frm_buf0, (IFloat *)gauge_field, (IFloat *)in, 
		     1, dag);
    wfm_dslash_end((IFloat *)frm_buf0, (IFloat *)gauge_field, (IFloat *)in, 
		   1, dag);
#else
    wfm_dslash((IFloat *)frm_buf0, (IFloat *)gauge_field, (IFloat *)in, 
		     1, dag);
#endif
#if 0
  {IFloat *tmp = (IFloat *)frm_buf0;
    ...
    wfm_dslash_begin((IFloat *)frm_buf0, (IFloat *)gauge_field, (IFloat *)out, 
		     0, dag);


  // PAB:
  // At the cost of another temporary the clover mutiply could
  // be overlapped with the communication in the dslash.

  // out = (Aoo - kappa*kappa Doe Aee^inv Deo) in
  //--------------------------------------------------------------------


    IFloat *ClovP = (IFloat *)lat.Aux1Ptr();
    clover_mat_mlt((IFloat *)clover_tmp, 
		   ClovP,
		   (const IFloat *)in, 
		   half_sites);    

    
    wfm_dslash_end((IFloat *)frm_buf0, (IFloat *)gauge_field, (IFloat *)out, 
		     0, dag);

    Float scale = -kappa *kappa;

    if ( dot_prd ) { 
      vaxpy3_norm(out,&scale,frm_buf0,clover_tmp,4*half_sites,dot_prd);
      CGflops += 24*half_sites*4;
    } else {
      vaxpy3(out,&scale,frm_buf0,clover_tmp,4*half_sites);
      CGflops += 24*half_sites*2;
    }
			  ...
			  PreserveType prs_in) 
{
  ...
  */
  {
    Float scale = kappa;
    vaxpy3(frm_buf1,&scale,frm_buf1,in,vec_size/6);
    CGflops+=vec_size*2;
  }
  ...
  */
  {
    Float scale = kappa;
    vaxpy3(frm_buf1,&scale,(Vector *)out_even, (Vector *)in_even,vec_size/6);
    CGflops += vec_size*2;
  }
