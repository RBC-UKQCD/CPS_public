...
//  CVS keywords
//
//  $Source: /home/chulwoo/CPS/repo/CVS/cps_only/cps_pp/src/util/dirac_op/d_op_mobius/sse/dwf_m_4dpc.C,v $
//  $State: Exp $
//
//--------------------------------------------------------------------
//------------------------------------------------------------------
		 ...
		 Dwf *dwf_lib_arg)
{
  ...
  //------------------------------------------------------------------
  time_elapse();
  dwf_dslash_4(out, gauge_field, in, 0, 0, dwf_lib_arg);
  DEBUG_DWF_DSLASH("dwf_dslash_4(o<-e) %e\n", time_elapse());

  //------------------------------------------------------------------
  // Apply M_5^-1 (hopping in 5th dir + diagonal)
  //------------------------------------------------------------------
  dwf_m5inv(out, mass, 0, dwf_lib_arg);
  DEBUG_DWF_DSLASH("dwf_m5inv %e\n", time_elapse());
  
  //------------------------------------------------------------------
  // Apply Dslash E <- O
  //------------------------------------------------------------------
  dwf_dslash_4(frm_tmp2, gauge_field, out, 1, 0, dwf_lib_arg);
  DEBUG_DWF_DSLASH("dwf_dslash_4(e<-o) %e\n", time_elapse());
  ...
  //my_moveFloat((IFloat*)out, (IFloat*)in, f_size);
#else
  ...
  DEBUG_DWF_DSLASH("out+= kap2 frm_tmp2 %e\n", time_elapse());
  ...
  //------------------------------------------------------------------
  dwf_dslash_5_plus_a_five(out, in, mass, 0, dwf_lib_arg, GJP.DwfA5Inv()*minus_kappa );
  DEBUG_DWF_DSLASH("dwf_dslash_5_plus %e\n", time_elapse());
