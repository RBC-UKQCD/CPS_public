...
CPS_START_NAMESPACE
/*! \file
  \brief  Definition of DiracOpNaive class methods.

  $Id: d_op_naive.C,v 1.2 2013-05-28 14:52:58 chulwoo Exp $
*/
//--------------------------------------------------------------------
//  CVS keywords
//
...
//  $Header: /home/chulwoo/CPS/repo/CVS/cps_only/cps_pp/src/util/dirac_op/d_op_naive/noarch/d_op_naive.C,v 1.2 2013-05-28 14:52:58 chulwoo Exp $
//  $Id: d_op_naive.C,v 1.2 2013-05-28 14:52:58 chulwoo Exp $
//  $Name: not supported by cvs2svn $
//  $Locker:  $
//  $Revision: 1.2 $
//  $Source: /home/chulwoo/CPS/repo/CVS/cps_only/cps_pp/src/util/dirac_op/d_op_naive/noarch/d_op_naive.C,v $
//  $State: Exp $
//
//--------------------------------------------------------------------
//------------------------------------------------------------------
...
// d_op_wilson.C
//
// DiracOpNaive is derived from the DiracOp base class. 
// DiracOpNaive is the front end for a library that contains
 ...
 */
//------------------------------------------------------------------
DiracOpNaive::DiracOpNaive(Lattice & latt,
			     Vector *f_field_out,
			     Vector *f_field_in,
			     CgArg *arg,
			     CnvFrmType convert) :
...
{
  cname = "DiracOpNaive";
  char *fname = "DiracOpNaive(L&,V*,V*,CgArg*,CnvFrmType)";
...
*/
//------------------------------------------------------------------
DiracOpNaive::~DiracOpNaive() {
  char *fname = "~DiracOpNaive()";
  VRB.Func(cname,fname);
...
// kappa.
//------------------------------------------------------------------
  void DiracOpNaive::DiracArg(CgArg *arg){
    dirac_arg = arg;
    //kappa = 1.0 / (2.0 * (dirac_arg->mass + 4.0));
    kappa = 1.0 / (2.0 * (dirac_arg->mass));
  }
...
//------------------------------------------------------------------
void DiracOpNaive::MatPcDagMatPc(Vector *out, 
					 Vector *in, 
					 Float *dot_prd){
  naive_mdagm((IFloat *)out, 
	       (IFloat *)gauge_field, 
	       (IFloat *)in, 
	       (IFloat *)dot_prd,
	       IFloat(kappa),
...
//------------------------------------------------------------------
void DiracOpNaive::Dslash(Vector *out, 
			   Vector *in, 
			   ChkbType cb, 
			   DagType dag) {
  naive_dslash((IFloat *)out, 
		(IFloat *)gauge_field, 
		(IFloat *)in, 
		int(cb),
		int(dag),
...
*/
//------------------------------------------------------------------
void DiracOpNaive::MatPc(Vector *out, Vector *in) {  
  naive_m((IFloat *)out, 
	   (IFloat *)gauge_field, 
	   (IFloat *)in, 
	   IFloat(kappa),
...
*/
//------------------------------------------------------------------
void DiracOpNaive::MatPcDag(Vector *out, Vector *in) {
  naive_mdag((IFloat *)out, 
	      (IFloat *)gauge_field, 
	      (IFloat *)in, 
	      IFloat(kappa),
	      (Wilson *)wilson_lib_arg);
...
//------------------------------------------------------------------
int DiracOpNaive::MatInv(Vector *out, 
			  Vector *in, 
			  Float *true_res,
			  PreserveType prs_in) {
...
#if 0
  Mat(in,out);
  for(int i=0;i<24*GJP.VolNodeSites();i++)
    if(!UniqueID())printf("MatMatinv Minv %d %e %e\n",i,*((Float*)in+i),*((Float*)out+i));
#endif
...
// but true_res=0.
//------------------------------------------------------------------
int DiracOpNaive::MatInv(Vector *out, Vector *in, PreserveType prs_in)
...
//------------------------------------------------------------------
int DiracOpNaive::MatInv(Float *true_res, PreserveType prs_in)
...
//------------------------------------------------------------------
int DiracOpNaive::MatInv(PreserveType prs_in)
{ return MatInv(f_out, f_in, 0, prs_in); }


//------------------------------------------------------------------

void DiracOpNaive::Mat(Vector *out, Vector *in) {  
  char *fname = "Mat(V*,V*)";
  VRB.Func(cname,fname);
...
void DiracOpNaive::MatDag(Vector *out, Vector *in) {
...
void DiracOpNaive::MatHerm(Vector *out, Vector *in) {
...
void DiracOpNaive::CalcHmdForceVecs(Vector *chi)
{
