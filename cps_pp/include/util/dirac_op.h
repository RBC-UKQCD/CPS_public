...
CPS_START_NAMESPACE typedef unsigned long long IntFlopCounter;
...
public:
  ...
  //eigen CG //by Qi Liu
  int InvEigCg (Vector * sol, Vector * src, Float * true_res, const int nev,
                const int m, Vector ** V, const int vec_len, Float * M,
                float **U, Rcomplex * invH, const int def_len,
                const Float * restart, const int restart_len);


  // Inverse matrix from eigen mode, for Low Mode Average
  int InvLowModeApprox (std::vector < Vector * >&out,
                        std::vector < Vector * >&in,
                        char *fname_eigenv,
                        int neig);

  // Inverse matrix from eigen mode, for Low Mode Average
  int InvLowModeApprox (Vector * out_p,
                        Vector * in_p,
                        char *fname_eigenv, int neig, Float * true_res)
  {

    std::vector < Vector * >out, in;
    out.push_back (out_p);
    in.push_back (in_p);
    *true_res=0.; 
    InvLowModeApprox (out, in, fname_eigenv, neig);
  }

  // lowmode projection onto src
  void InvLowModeProj (Vector * in, char *fname_eigenv, int neig);

  //! The matrix inversion used in the molecular dynamics algorithms.
  int InvCgShift (Vector * out,
                  Vector * in,
                  Float src_norm_sq, Float * true_res, Float * shift);

  int InvCg (Vector * out, Vector * in, Float src_norm_sq, Float * true_res);
  ...
  virtual int ImpResLanczos (Vector ** V,       //Lanczos vectors, eigenvectors of RitzMat on return
                             ...
                             LanczosArg * lanczos_arg); // arguments for eigen solver algorithm
  //
  ...
  void lanczos (int k, int m, size_t f_size,
                Vector * Apsi, Vector * r, Vector ** V, Float * alpha,
                Float * beta, MatrixPolynomialArg * cheby_arg,
                RitzMatType RitMat_lanczos);



  //! Vector orthogonalisation
  void GramSchm (Vector ** psi, int Npsi, Vector ** vec, int Nvec,
                 size_t f_size);
  ...
  int Jacobi (Vector ** psi, int N_eig, Float * lambda,
              Complex * off_diag, Float Toler, int N_max);
 ...
 */
   ...
   */
  virtual void MatPcDagMatPc (Vector * out, Vector * in, Float * dot_prd = 0) =
    0;
   ...
   */
  virtual void Dslash (Vector * out, Vector * in, ChkbType cb, DagType dag) = 0;
  ...
  virtual int MatInv (Vector * out,
                      Vector * in,
                      Float * true_res, PreserveType prs_in = PRESERVE_YES) = 0;
  ...
  virtual int eig_MatInv (Vector ** V, const int vec_len, Float * M,
                          const int nev, const int m, float **U,
                          Rcomplex * invH, const int def_len,
                          const Float * restart, const int restart_len,
                          Vector * out, Vector * in, Float * true_res,
                          PreserveType prs_in = PRESERVE_YES) {
    const char *fname = "eig_MatInv()";
    ERR.General (cname, fname,
                 "only implemented for dwf currently, so not pure virtual\n");
    return 0;
  }
  ...
  /*!
     ...
     The residual is  <em>  |b - A x| / |b| </em>
   */
  virtual int MatInv (Float * true_res, PreserveType prs_in = PRESERVE_YES) = 0;
...
public:
  ...
  void Dslash (Vector * out,
               Vector * in, ChkbType cb, DagType dag, int dir_flag);
  // Dslash is the derivative part of the fermion matrix. 
  // Dslash conects only odd-->even or even-->odd sites.
  // The in, out fields are defined on a checkerboard.
  // cb refers to the checkerboard of the in field.
  ...
  int MatInv (Vector * out,
              Vector * in,
              Float * true_res, PreserveType prs_in = PRESERVE_YES);
...
public:
  ...
  void dMdmu (Vector * out, Vector * in, ChkbType cb, DagType dag, int order);
...
//  {ERR.NotImplemented(cname,"RitzMat(V*,V*)");}
  ...
  int BiCGstab (Vector * out,
                Vector * in, Float src_norm_sq, int n, Float * true_res);
  // The Conjugate Gradient inverter. 
  // Source is *in, initial guess and solution is *out.
  // src_norm_sq is the norm of the source squared 
  // (dot product of source * source). If src_norm_sq=0 
...
public:
  const Float & get_epsilon () const
  {
    return epsilon;
  }
  const Float & get_ctheta () const
  {
    return ctheta;
  }
  const Float & get_stheta () const
  {
    return stheta;
  }
  const Float & get_kappa () const
  {
    return kappa;
  }
...
//
  ...
  void Dslash_tm (Vector * out, Vector * in, ChkbType cb, DagType dag);
//
  void MatPc (Vector * out, Vector * in);
...
//
  ...
  int RitzEig (Vector ** eigenv, Float lambda[], int valid_eig[],
               EigArg * eig_arg);
};
...
public:
  ...
  void MatPcDagOrNot (Vector * out, const Vector * in, int dag, Float * dot =
                      NULL) const;

  // FOR THE PURPOSE OF DEBUGGING ONLY
  //----------------------------------------------------------------------
//! For  debugging  purposes only, apparently
  void MatDagOrNotDbg (Vector * out, const Vector * in, int dag,
                       int direct) const;
  // out = MatDag in    OR  out = Mat in

//! For  debugging  purposes only, apparently
  void MatDagMatDbg (Vector * out, Vector * in, Float * dot_prd =
                     0, int direct = 0);
  // out = MatDagMat in
...
public:
  ...
  void Dslash (Vector * out, Vector * in, DagType dag);
  ...
  // The function returns the total number of CG iterations.
  //
  void MatInv (std::vector<Vector *> out, std::vector<Vector *> in,
              std::vector<Float> &true_res, std::vector<int> &iters);
  ...
  int QudaInvert (Vector * out, Vector * in, Float * true_res, int mat_type);
  int QudaInvert (std::vector<Vector *> out, std::vector<Vector *> in,
              std::vector<Float> &true_res, std::vector<int> &iters, int mat_type);
...
public:
  ...
  void MatDag (Vector * out, Vector * in)
  {
    ERR.NotImplemented (cname, "MatDag(V*,V*)");
  }
  void MatPc (Vector * out, Vector * in)
  {
    ERR.NotImplemented (cname, "MatPc(V*,V*)");
  }
  void MatPcDagMatPc (Vector * out, Vector * in, Float * dot_prd = 0) {
    ERR.NotImplemented (cname, "MatPcDagMatPc(V*,V*,F*)");
  }
  void Dslash (Vector * out, Vector * in, ChkbType cb, DagType dag)
  {
    ERR.NotImplemented (cname, "Dslash(V*,V*,ChkbType,DagType");
  }
  int MatInv (Vector * out, Vector * in, Float * true_res, PreserveType prs_in =
              PRESERVE_YES) {
    ERR.NotImplemented (cname, "MatInv(V*,V*,F*,PreserveType)");
    return 0;
  }
  int MatInv (Vector * out, Vector * in, PreserveType prs_in = PRESERVE_YES) {
    ERR.NotImplemented (cname, "MatInv(V*,V*,PreserveType)");
    return 0;
  }
  int MatInv (Float * true_res, PreserveType prs_in = PRESERVE_YES) {
    ERR.NotImplemented (cname, "MatInv(F*,PreserveType)");
    return 0;
  }
  int MatInv (PreserveType prs_in = PRESERVE_YES) {
    ERR.NotImplemented (cname, "MatInv(PreserveType)");
    return 0;
  }
#ifdef USE_QUDA
  ...
  void Mat (Vector * out, Vector * in);
  void MatHerm (Vector * out, Vector * in);
  void MinResExt (Vector * psi, Vector * phi, Vector ** psi_old, Vector ** vm,
                  int degree) override;
  void CalcHmdForceVecs (Vector * v1, Vector * v2, Vector * phi1,
                         Vector * phi2);
  int QudaInvert (Vector * out, Vector * in, Float * true_res, int mat_type);
#endif
