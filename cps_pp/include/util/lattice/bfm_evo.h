...
void bisec (std::vector < double >alpha, std::vector < double >beta, int n,
	    ...
	    std::vector < double >&x, int *z, double *eps2);
enum
{ Export = 0, Import = 1 };
...
// shifts/conjugates ......
template < class Float > class bfm_evo:public bfm_qdp < Float >
{
public:
  // BFM has this
  // enum {Even = 0, Odd};
...
private:
  ...
  void fforce_surface (Float * mom, Float * gauge, Float * v1, Float * v2,	// internal data
  ...
  // complex version of axpy()
#if 0
  void axpy_c (Fermion_t r, Fermion_t x, Fermion_t y,
	       std::complex < double >a, Fermion_t tmp)
  {
    this->zaxpy (r, x, y, a);
  }
#endif
...
public:
  ...
  template < typename FloatEXT >
    void thread_impexFermion_s (FloatEXT * psi, Fermion_t handle[2],
				int doimport);
...
public:
  ...
  template < typename FloatEXT >
    void cps_impexcbFermion (FloatEXT * psi, Fermion_t handle, int doimport,
			     int cb);
  ...
  // mainly used to import/export the "v1" and "v2" vectors in evolution.
  template < typename FloatEXT >
    void cps_impexFermion_s (FloatEXT * psi, Fermion_t handle[2],
			     int doimport);
  ...
  double CompactMprec (Fermion_t compact_psi,
		       ...
		       Fermion_t psi,
		       Fermion_t chi, Fermion_t tmp, int dag, int donrm = 0);

  // copied from Jianglei's bfm
  void CompactMunprec (Fermion_t compact_psi[2],
		       Fermion_t compact_chi[2],
		       Fermion_t psi[2],
		       Fermion_t chi[2], Fermion_t tmp, int dag);
					 ...
					 int i_size)
{
  ...
  int csite
    ...
    * (x[1] + this->node_latt[1]
       * (x[2] + this->node_latt[2] * (x[3] + s * this->node_latt[3])));
  csite /= 2;
...
template < class Float > void bfm_evo < Float >::threadedFreeFloat (Float * f)
{
  int me = this->thread_barrier ();
  if (me == 0)
    {
...
static inline int
idx_4d (const int x[4], const int lx[4])
{
  int ret = 0;
  for (int i = 3; i >= 0; --i)
    {
...
static inline int
idx_5d (const int x[5], const int lx[5])
{
  int ret = 0;
  for (int i = 4; i >= 0; --i)
    {
...
static inline int
idx_4d_surf (const int x[4], const int lx[4], int mu)
{
  int ret = 0;
  for (int i = 3; i >= 0; --i)
    {
...
static inline int
idx_5d_surf (const int x[5], const int lx[5], int mu)
{
  int ret = 0;
  for (int i = 4; i >= 0; --i)
    {
	  ...
	  if (s == 0)
	    {
	      ...
	      this->axpby_ssp_proj (chi, b_coefs[s], psi, -c_coefs[s], psi, s,
				    s + 1, Pminus);
	      this->axpby_ssp_proj (chi, 1.0, chi, this->mass * c_coefs[s],
				    psi, s, this->Ls - 1, Pplus);
	    }
	  else if (s == (this->Ls - 1))
	    {
	      this->axpby_ssp_proj (chi, b_coefs[s], psi,
				    this->mass * c_coefs[s], psi, s, 0,
				    Pminus);
	      this->axpby_ssp_proj (chi, 1.0, chi, -c_coefs[s], psi, s, s - 1,
				    Pplus);
	    }
	  else
	    {
...
quad_solve (double *ct, double *st,
...
{
  int me = this->thread_barrier ();
  Fermion_t tmp0[2], old_x[2], tmp1[2];
  tmp0[0] = this->threadedAllocFermion ();
  tmp0[1] = this->threadedAllocFermion ();
  tmp1[0] = this->threadedAllocFermion ();
  tmp1[1] = this->threadedAllocFermion ();
  old_x[0] = this->threadedAllocFermion ();
  old_x[1] = this->threadedAllocFermion ();
  int nprec = 1;
  if (this->solver == DWFKernel)
    {
      nprec = 2;
    }

  // normalize x
  double fact = this->norm (x[0]);
  for (int i = 1; i < nprec; i++)
    fact += this->norm (x[i]);
  fact = sqrt (1. / fact);
  ...
  if (this->isBoss () && !me)
    {
      printf ("bfm_evo::simple_lanczos nprec = %d <x, x> = %17.10e\n", nprec,
	      1. / (fact * fact));
    }
//  CJ FIXME: add one to index to conform with bisec(), Change later!!
  ...
  for (int i = 0; i < nprec; i++)
    this->copy (old_x[i], x[i]);
  double old_low = 0., old_hi = 0.;	// being lazy. maybe ok?
  for (int iter = 0; iter < this->max_iter; ++iter)
    {
      double rel_low = 1., rel_hi = 1.;
      switch (this->solver)
	{
	case DWFKernel:
	  ...
	  break;
	default:
//    case HmCayleyComplex  :
//    case HmCayleyTanh  :
	  // y = A x, A = MdagM or -MdagM
	  this->Mprec (x[0], tmp0[1], tmp1[0], 0, 1);
	  me = this->thread_barrier ();
	  this->Mprec (tmp0[1], tmp0[0], tmp1[1], 1);
	  me = this->thread_barrier ();
	  break;
//      if(this->isBoss() && !me) printf("simple_lanczos not defined for solver %s\n",this->SolverString(this->solver));
//      exit(-4);
//    break;
	}
      std::complex < Float > inner0 = this->dot (x[0], tmp0[0]);
      for (int i = 1; i < nprec; i++)
	inner0 += this->dot (x[i], tmp0[i]);
      std::complex < Float > inner1 = this->inner (x[0], tmp0[0]);
      for (int i = 1; i < nprec; i++)
	inner1 += this->inner (x[i], tmp0[i]);

      me = this->thread_barrier ();
      if (iter == 0)
	if (this->isBoss () && !me)
	  {
	    printf ("bfm_evo::simple_lanczos alpha[%d] = %e %e %e %e\n", iter,
		    inner0.real (), inner0.imag (), inner1.real (),
		    inner1.imag ());
	  }
      alpha[iter + 1] = inner0.real ();
      fact = this->axpby_norm (tmp0[0], tmp0[0], x[0], 1., -alpha[iter + 1]);
      for (int i = 1; i < nprec; i++)
	fact +=
	  this->axpby_norm (tmp0[i], tmp0[i], x[i], 1., -alpha[iter + 1]);
      if (iter > 0)
	{
	  fact =
	    this->axpby_norm (tmp0[0], tmp0[0], old_x[0], 1.,
			      -beta[iter + 1]);
	  for (int i = 1; i < nprec; i++)
	    fact +=
	      this->axpby_norm (tmp0[i], tmp0[i], old_x[i], 1.,
				-beta[iter + 1]);
	}

      inner1 = this->inner (old_x[0], tmp0[0]);
      for (int i = 1; i < nprec; i++)
	inner1 += this->inner (old_x[i], tmp0[i]);
      inner0 = this->inner (old_x[0], x[0]);
      for (int i = 1; i < nprec; i++)
	inner0 += this->inner (old_x[i], x[i]);
      if (iter == 0)
	if (this->isBoss () && !me)
	  {
	    printf
	      ("bfm_evo::simple_lanczos <old_x|A|old_x> <old_x|x> = %e %e %e %e\n",
	       iter, inner1.real (), inner1.imag (), inner0.real (),
	       inner1.real ());
	  }


      beta[iter + 2] = sqrt (fact);
      fact = sqrt (1. / fact);
      for (int i = 0; i < nprec; i++)
	this->scale (tmp0[i], fact);

      for (int i = 0; i < nprec; i++)
	this->copy (old_x[i], x[i]);
      for (int i = 0; i < nprec; i++)
	this->copy (x[i], tmp0[i]);
...
//void bisec(std::vector<double>  alpha ,std::vector<double> beta,int n,int m1,int m2,double eps1,double relfeh,std::vector<double> &x,int *z,double *eps2);
	  if ((iter % 10 == 0) && (iter > 0))
		...
		printf
	      ...
	      if (this->isBoss () && (!me))
		printf
		  ("bfm_evo::simple_lanczos %d: lowest(1e-14)= %17.10e  %17.10e  %17.10e\n",
		   iter, eigs[1], eigs[2], eigs[3]);
	      bisec (alpha, beta, iter, iter - 2, iter, 1e-14, 1e-14, eigs,
		     &z, &eps);
	      if (this->isBoss () && (!me))
		printf
		  ("bfm_evo::simple_lanczos %d: highest(1e-14)= %17.10e %17.10e %17.10e\n",
		   iter, eigs[iter - 2], eigs[iter - 1], eigs[iter]);
      ...
      else
	{
      ...
      me = this->thread_barrier ();
//          this->thread_sum(rel_low,me);
//          this->thread_sum(rel_hi,me);
      if ((iter % 10 == 0) && (iter > 0))
	{
	  if (this->isBoss () & (!me))
	    printf
  ...
  double bfm_evo < Float >::ritz (Fermion_t x, int compute_min)
{
  ...
  if (this->isBoss () && !me)
      ...
      else
	{
      ...
      printf
	("bfm_evo::ritz three ways of computing the eigenvalue should agree.\n");
...
template < class Float >
  void bfm_evo < Float >::copySendFrmData (Float v3d[], Float v4d[], int mu,
					   bool send_neg)
{
...
template < class Float >
  void bfm_evo < Float >::compute_force (Float * mom, Float * gauge,
					 Fermion_t phiL, Fermion_t phiR,
					 double coef)
{
  int me = this->thread_barrier ();

  Fermion_t v1[2] =
    { this->threadedAllocFermion (), this->threadedAllocFermion () };
  Fermion_t v2[2] =
    { this->threadedAllocFermion (), this->threadedAllocFermion () };
  ...
  int lclx[5] = { this->node_latt[0], this->node_latt[1], this->node_latt[2],
    this->node_latt[3], this->Ls
  };
  ...
  for (int i = 0; i < 4; ++i)
    {
      fforce_surface (mom, gauge, v1f, v2f, rcvbuf + surf_v1[i],	// v1 surface
  ...
  int bfm_evo < Float >::bicg_M (Fermion_t sol, Fermion_t src)
{
  ...
  for (; k <= this->max_iter; ++k)
	  ...
	  if (this->isBoss () && !me)
	    {
	      ...
	      printf ("bicg_M: acc_rsd = %9.3e %9.3e true_rsd = %9.3e\n",
		      sqrt (rnorm / src_norm), sqrt (rdnorm / src_norm),
	...
	{
	  printf
	    ("iter = %5d rsd = %9.3e true rsd = %9.3e a = (%9.3e %9.3e) b = (%9.3e %9.3e)\n",
	     k, rnorm, true_rsd, real (alpha), imag (alpha), real (beta),
	     imag (beta));
	}
      // ======================================================================
    }
  ...
  for (; k <= this->max_iter; ++k)
      ...
      if (this->isBoss () && !me)
	{
	  printf ("iter = %5d true rsd = %10.3e "
		  ...
		  k, true_rsd,
		  real (rho), imag (rho),
		  real (alpha), imag (alpha), real (omega), imag (omega));
	}
      // ======================================================================
    }
...
template < typename Float >
  double bfm_evo < Float >::CompactMprec (Fermion_t compact_psi,
					  ...
					  Fermion_t psi,
					  Fermion_t chi,
					  Fermion_t tmp, int dag, int donrm)
{
  ...
  for (; j < this->max_iter / m; ++j)
      ...
      for (int i = 0; i < m; ++i)
	{
	  ...
	  for (int k = 1; k <= i; ++k)
	    {
	      cmplx gamma =
		C[k - 1] * R[(k - 1) * m + i] + conj (S[k - 1]) * H[k * m +
								    i];
	      R[k * m + i] =
		-S[k - 1] * R[(k - 1) * m + i] + C[k - 1] * H[k * m + i];
	      R[(k - 1) * m + i] = gamma;
	    }
	  ...
	  R[i * m + i] =
	    C[i] * R[i * m + i] + conj (S[i]) * H[(i + 1) * m + i];
	  B[i + 1] = -S[i] * B[i];
	  B[i] *= C[i];
