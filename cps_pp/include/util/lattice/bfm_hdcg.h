...
int bfm_evo<Float>::HD_CGNE_prec(Fermion_t psi, Fermion_t src)
{
    ...
    HDCGController<Float>* hdcg = HDController<Float>::getInstance();

    assert(hdcg->get_vec_len() == this->cbLs*24*this->node_cbvol);
...
//    complex<double> *invH=NULL;
    if(hdcg->def_len > 0) {
        ...
        if(hdcg->def_len<hdcg->get_max_def_len()) {
            this->thread_barrier();
            //------
        } else {
            invH = hdcg->H;
        ...
        Ub = (complex<double>*)threaded_alloc(hdcg->def_len*sizeof(complex<double>));
        invHUb = (complex<double>*)threaded_alloc(hdcg->def_len*sizeof(complex<double>));
        //Do initial projection
        //low modes guess = U*invH*U^dag*src;
        for(int n=0; n<hdcg->def_len; n++) {
            complex<double> dt = this->inner((Fermion_t)(hdcg->getU(n)), src);
            if(me==0)Ub[n] = dt;
        }
        if(me==0) {
            for(int i=0;i<hdcg->def_len;i++) {
                invHUb[i]=0.0;
                for(int j=0;j<hdcg->def_len;j++)
                    invHUb[i] += invH[i*hdcg->def_len+j]*Ub[j]; //notice the index ofr invH.
            }
        }
        ...
        this->set_zero(tmp);
        for(int i=0;i<hdcg->def_len;i++) {
            this->zaxpy(tmp, (Fermion_t)(hdcg->getU(i)), tmp, invHUb[i]); 
            // this->copy(tmp,(Fermion_t)(hdcg->getU(i)));
    ...
    if ( cp <= rsq ) {
        ...
        if(hdcg->def_len>0 && hdcg->def_len<hdcg->get_max_def_len()) {
            threaded_free(invH);
        }
        return 0;
    }
    ...
    int restart_pos = 0;
    if(hdcg->restart.size()>0) {
        restart_rsq = hdcg->restart[restart_pos]*hdcg->restart[restart_pos]*ssq;
    }

    const bool do_hdcg = (hdcg->get_nev()>0 && hdcg->def_len<hdcg->get_max_def_len());
    int i_eig = 0;
    a = 1.0;
    b = 0.0;
    double a_old = a;
    ...
    for (int k=1;k<=this->max_iter;k++) {
        ...
        if(do_hdcg) {
            ...
            const int m = hdcg->get_m();
            const int nev=hdcg->get_nev();
            ...
            if(i_eig==m) {
                ...
                // eigcg_vec_mult(hdcg->getV(0),m,QZ,rank,hdcg->get_vec_len(), this->nthread, me);
                eigcg_vec_mult2(hdcg->getV(0),m,QZ,rank,hdcg->get_vec_len(), this->nthread, me, *this);
                // eigcg_vec_mult3(hdcg->getV(0),m,QZ,rank,hdcg->get_vec_len(), this->nthread, me, *this);

                eigProj_flops += 2*hdcg->get_vec_len()*m*rank;
                ...
                for(int ii=0;ii<i_eig;ii++) {
                    //T(i_eig,ii)
                    double dt=this->inner_real(mmp_prev,hdcg->getV(ii));
                    if(me==0) {
            ...
            //V[i_eig]=r/sqrt(rsq);
            this->copy(hdcg->getV(i_eig),r);
            this->scale(hdcg->getV(i_eig),1.0/std::sqrt(cp));
            i_eig++;
        ...
        //restart condition
        if(cp <= restart_rsq && hdcg->def_len >0 && (hdcg->def_len==hdcg->get_max_def_len() || hdcg->is_always_restart())) {
            struct timeval proj_start,proj_end,proj_diff;
            gettimeofday(&proj_start,NULL);
            restart_pos++;
            if(restart_pos >= hdcg->restart.size())restart_rsq=0.0;
            else restart_rsq = hdcg->restart[restart_pos]*hdcg->restart[restart_pos]*ssq;
            complex<double> *Ub = NULL;
            complex<double> *invHUb = NULL;
            Ub = (complex<double>*)threaded_alloc(hdcg->def_len*sizeof(complex<double>));
            invHUb = (complex<double>*)threaded_alloc(hdcg->def_len*sizeof(complex<double>));
            ...
            for(int n=0; n<hdcg->def_len; n++) {
                complex<double> dt = this->inner((Fermion_t)(hdcg->getU(n)), r);
                if(me==0)Ub[n] = dt;
            }
            if(me==0) {
                for(int i=0;i<hdcg->def_len;i++) {
            ...
            for(int i=0;i<hdcg->def_len;i++) {
                this->zaxpy(tmp, (Fermion_t)(hdcg->getU(i)), tmp, invHUb[i]);
                // this->copy(mp,(Fermion_t)(hdcg->getU(i)));
    ...
    if(do_hdcg) {
        ...
        int *low = new int[2*hdcg->get_nev()];
        //add low modes from the lowest first
        //sort
        int s;
        for(s=0;s<2*hdcg->get_nev();s++)low[s]=s;
        for(int i=0;i<2*hdcg->get_nev()-1;i++) {
            for(int j=2*hdcg->get_nev()-1;j>i;j--) {
                if(M[low[j]]<M[low[j-1]]){s=low[j];low[j]=low[j-1];low[j-1]=s;}
            }
        }

        //add
        for(int i=0;i<2*hdcg->get_nev();i++) {
            if(this->isBoss() && this->verbose && !me ) {
                printf("eigen value %d is M[%d] = %e \n",i, low[i], M[low[i]]);
            }
            //update deflation space U from V
            if(M[low[i]]<hdcg->get_max_eig_cut() && M[low[i]]>1e-20 && hdcg->def_len<hdcg->get_max_def_len()) {
                ...
                this->copy((Fermion_t)hdcg->getU(hdcg->def_len), (Fermion_t)hdcg->getV(low[i]));
                this->copy(p,(Fermion_t)hdcg->getV(low[i]));
                ...
                for(int j=0;j<hdcg->def_len;j++) {
                    complex<double> dt = this->inner((Fermion_t)(hdcg->getU(j)),mmp);
                    if(me==0)hdcg->H[j*hdcg->get_max_def_len()+hdcg->def_len]=dt;
                }
                if(me==0) {
                    for(int j=0;j<hdcg->def_len;j++) {
                        hdcg->H[hdcg->def_len*hdcg->get_max_def_len()+j]
                            =std::conj(hdcg->H[j*hdcg->get_max_def_len()+hdcg->def_len]);
                    }
                }
                complex<double> dt = this->inner((Fermion_t)(hdcg->getV(low[i])),mmp);
                if(me==0) {
                    hdcg->H[hdcg->def_len*hdcg->get_max_def_len()+hdcg->def_len] =  dt;
                    hdcg->def_len++;	
                }
                if(this->isBoss() && this->verbose && !me) {
                    printf("def_len = %d\n",hdcg->def_len);
        ...
        if(me==0) {
            if(hdcg->def_len == hdcg->get_max_def_len())
                invert_H_matrix(hdcg->H,hdcg->get_max_def_len());
        }
