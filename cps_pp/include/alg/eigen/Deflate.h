#ifndef BFM_DEFLATE_H
#define BFM_DEFLATE_H
...
namespace BFM_Krylov{
  ...
  public:
    ...
    **/
    static T orthQ(Matrix<T> &Q, std::vector<T> y){
    ...
    /**
       ...
       Q.e_1 = y and Q is unitary.
    **/
    static double orthQM(Matrix<T> &Q, Matrix<T> &H, std::vector<T> y){
	...
	if(abs(tau0) > 0.0){	
	  ...
	  if(j < (N-1) && abs(tau) < 0.05){
	    ...
	    T alpha = 0.0; std::vector<T> tmp(j+1); for(int k=0;k<j+1;k++){tmp[k] = Q(k,j);}
	    Matrix<T> Hj(j+1); Hj = H.GetSubMtx(0, j+1, 0, j+1);
	    std::vector<T> tmp2(j+1); tmp2 = Hj*tmp; for(int k=0;k<j+1;k++){alpha+=conj(y[k])*tmp2[k];} //TODO
    ...
    **/
    static T orthU(Matrix<T> &Q, std::vector<T> y){
    ...
    **/
    ...
    static void Lock(Matrix<T> &H, 	///Hess mtx	
      ...
      int M = H.dim;
      std::vector<T> vec(M-con);
      Matrix<T> AH(M-con);
      AH = H.GetSubMtx(con, M, con, M);
      Matrix<T> QQ(M-con);
      Q.Unity(); QQ.Unity();
	
      std::vector<T> evals(M-con);
      std::vector<std::vector<T> > evecs(M-con);
	...
	Matrix<T> U(j-1-con);
	std::vector<T> z(j-1-con); T nm = norm(z); for(int k = con+0;k<j-1;k++){z[k-con] = conj( H(j,k+1) );}
	normalize(z);
	...
	std::vector<std::vector<T> > Hb(j-1-con);	
	for(int k=con+1;k<j;k++){Hb[k-1-con].resize(M);}
	
	for(int a = 0;a<M;a++){
	  for(int b = 0;b<j-1-con;b++){T sum = 0;
	    for(int c = 0;c<j-1-con;c++){sum += H(a,con+1+c)*U(c,b);}//sum += H(a,con+1+c)*U(c,b);}
	    Hb[b][a] = sum;}}
	
	for(int k=con+1;k<j;k++){
	  for(int l=0;l<M;l++){H(l,k) = Hb[k-1-con][l];}}//H(Hb[k-1-con][l] , l,k);}}

	std::vector<std::vector<T> > Qb(j-1-con);	
	for(int k=con+1;k<j;k++){Qb[k-1-con].resize(M);} 
	
	for(int a = 0;a<M;a++){
	  for(int b = 0;b<j-1-con;b++){T sum = 0;
	    for(int c = 0;c<j-1-con;c++){sum += Q(a,con+1+c)*U(c,b);}//sum += Q(a,con+1+c)*U(c,b);}
	    Qb[b][a] = sum;}}
	
	for(int k=con+1;k<j;k++){
	  for(int l=0;l<M;l++){Q(l,k) = Qb[k-1-con][l];}}//Q(Qb[k-1-con][l] , l,k);}}

	std::vector<std::vector<T> > Hc(M);	
	for(int k=0;k<M;k++){Hc[k].resize(j-1-con);}
	
	
	for(int a = 0;a<j-1-con;a++){
	  for(int b = 0;b<M;b++){T sum = 0;
	    for(int c = 0;c<j-1-con;c++){sum += conj( U(c,a) )*H(con+1+c,b);}//sum += conj( U(c,a) )*H(con+1+c,b);}
	    Hc[b][a] = sum;}}
	
	for(int k=0;k<M;k++){
	  for(int l=con+1;l<j;l++){H(l,k) = Hc[k][l-1-con];}}//H(Hc[k][l-1-con] , l,k);}}
