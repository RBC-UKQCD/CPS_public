...
//  const int num_nodes=do_arg.x_nodes*do_arg.y_nodes*do_arg.z_nodes*do_arg.t_nodes*do_arg.s_nodes;
  ...
  char *prefix = CommandLine::arg();
  Float stp_cnd(CommandLine::arg_as_Float());
  ...
  else if (strcmp(lat_stand_in,"do_arg")==0){
    VRB.Result(cname,fname,"Already read via do_arg");
  }
  else {
    if (strcmp(lat_stand_in,rotated_lat_fname)==0){
      VRB.Result(cname,fname,"  Lattice name(%s) suggests already gauge fixed\n",lat_stand_in);
      lat_gfixed=1;
    }
  ...
  if (chkpoints)
    ...
    if (!lat_gfixed) fix_arg.max_iter_num = 10000;
    else fix_arg.max_iter_num = 0;
    VRB.Result(cname,fname,"  fix_arg.max_iter_num = %d\n",fix_arg.max_iter_num);
  ...
  qpropw_arg.cg.stop_rsd=stp_cnd;
  qpropw_arg.cg.true_rsd=stp_cnd;
...
//    ERR.FileA("main","main", (char *)(logfile) );
  ...
  for (int i=0; i<num_masses; i++) {
	...
	if (chkpoints)
	  chkpt(num_nodes,chkpoint_no,dtime,dtime_size);
	sprintf(prop_name,"%s_traj%d_m%0.5f_tshift%d_%s",prefix,seqNum,masses[i],-t_src,bc_label); //Note that t_src is w.r.t. the unshifted lattice in the file name, but the source time in the qio header will always say 0 because it is equal to qpropw_arg.t
