 ...
 */
  ...
  // debug
  printf("global Ls : %d, DwfHeight : %f, DwfA5Inv : %f\n", 
	 GJP.SnodeSites(), GJP.DwfHeight(), GJP.DwfA5Inv());
...
//  const int num_nodes=do_arg.x_nodes*do_arg.y_nodes*do_arg.z_nodes*do_arg.t_nodes*do_arg.s_nodes;
  ...
  int max_cg_iter(CommandLine::arg_as_int() );
...
#pragma omp parallel
  ...
  qpropw_arg.cg.max_num_iter=max_cg_iter;
  ...
  char logfile[200];
  sprintf(logfile,"%s/%d.mdone",logdir,seqNum);
...
//    ERR.FileA("main","main", (char *)(logfile) );
  ...
  for (int i=0; i<num_masses; i++) {
	...
	sprintf(midprop_contractions_fname,"midprop_contractions.%s",prop_name);
	...
	sprintf(qio_out,"qio_prop.%s",prop_name);
#endif
