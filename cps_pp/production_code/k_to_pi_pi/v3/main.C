 ...
 */
  ...
  char contractions_done_list[200], light_prop_dir[200];
  ...
  sprintf(light_prop_dir, CommandLine::arg() );
  ...
  for (int i_light=0; i_light<num_light; i_light++) {
    sprintf(wme_results[i_light],"%s.%d",threept_arg.results,seqNum);
    sprintf(wme_results_mres_ZA[i_light],"%s.%d",threept_arg.results_mres_ZA,seqNum);
    sprintf(wme_results_pipi[i_light],"%s.%d",threept_arg.results_pipi,seqNum);
  }
  ...
  for (int i_light=0; i_light<1; i_light++) {
	  ...
	  for (int src_num=0; src_num<num_src_times; src_num++) {
		...
		for (mom_num=0; mom_num<n_mom; mom_num++) {
		  ...
		  for (mom_dir=0; mom_dir<n_dir; mom_dir++) {
		    ...
		    sprintf(prop_name_w_config_no,"%s_traj%d_m%0.5f_tshift%d_%s%s%s",label,seqNum,this_mass,-t_src,source_info,bc_label,mom_info); 
//Note that the variable t_src is w.r.t. the unshifted lattice in the file name, whereas the source time in source_info refers to the value of qpropw_arg.t which is what will appear in the qio header
		    if (overwrite_previous_config_props)
		      sprintf(prop_name,"%s_m%0.5f_tshift%d_%s%s%s",label,this_mass,-t_src,source_info,bc_label,mom_info);
		    else
...
// quark without twist
			else sprintf(qio_filename,"%s/%d/qio_prop.%s",light_prop_dir,seqNum,prop_name);
			...
			if (!testval2) { //Only need to reload if not already in memory.
			  ...
			  if (qpropw_arg.save_prop) {
			    ...
			    Fclose(fp);
			    if (!q_type){
			      VRB.Result("main","main","hard-wired to stop after every light quark inversion to avoid going over ANL queue time limit");
//			      exit(-5);
			      return 0;
			    }
			  }
