...
//    AlgActionRationalQuotient rat_quo(mom, rat_quo_arg);
  ...
  Float upper = quo_arg.quotients.quotients_val[0].bsn_mass_epsilon;
  Float lower = quo_arg.quotients.quotients_val[0].frm_mass_epsilon;
  VRB.Result(cname,fname,"bsn_mass_epsilon=%g frm_mass_epsilon=%g n_step=%d n_mass=%d\n",
...
//    LRG.Read(rng_file,32);
    ...
    for(int i =0;i<n_step;i++){
      ...
      for(int j = 0;j<n_mass;j++){
        quo_arg.quotients.quotients_val[j].bsn_mass_epsilon  = b_mass;
        quo_arg.quotients.quotients_val[j].frm_mass_epsilon  = f_mass;
      }
