             ...
             const char fn_t[], int traj)
{
    ...
    run_wall_snk(&lwsnk, lprop);

    // truncate files
    FILE *fp[total_ops];
    for(int op_id = 0; op_id < total_ops; ++op_id) {
        char buf[1024];
        sprintf(buf, fn_t, op_str[op_id], traj);
        fp[op_id] = Fopen(buf, "w");
    }

    for(unsigned sep = 0; sep < t_size_glb; ++sep) {
        for(unsigned tk = 0; tk < t_size_glb / 2; ++tk) {
            const unsigned tp = (tk + sep) % t_size_glb;
            if(sprop.empty(tk)) continue;
            if(lwsnk[tk].empty()) continue;
            if(ltwst.empty(tp)) continue;
            // 10 operators, see previous list
            vector<vector<Rcomplex> > kl3(total_ops, vector<Rcomplex>(t_size_glb, Rcomplex(0, 0)));

#pragma omp parallel
            {
                // threaded results
                vector<vector<Rcomplex> > tmp(total_ops, vector<Rcomplex>(t_size_glb, Rcomplex(0, 0)));
...
#pragma omp for
                for(int i = 0; i < 2 * lcl_vol; ++i) {
                    ...
                    WilsonMatrix p[3] = {0, ltwst[tp][i], 0};
                    p[0].glV(lwsnk[tk][tp], -5);
                    p[2].glV(sprop[tk][i], -5);
                    p[2].hconj();
...
#pragma omp critical
                for(int op_id = 0; op_id < total_ops; ++op_id) {
                    for(int t = 0; t < t_size_glb; ++t) {
                        kl3[op_id][t] += tmp[op_id][(t+tk)%t_size_glb];
                    }
                } // critical, for
            }//omp
            ...
            assert(GJP.Snodes() == 1);
            for(int op_id = 0; op_id < total_ops; ++op_id) {
                QMP_sum_double_array((double *)kl3[op_id].data(), 2 * t_size_glb);

                const double norm = 1.;
                // double norm = 1. / t_size_glb;
