                  ...
                  const char fn_t[], int traj)
{
    ...
    for(unsigned k = 0; k < t_size_glb; ++k) {
        if(propA.empty(k)) continue;
        if(propB.empty(k)) continue;

        vector<Rcomplex> meson(t_size_glb, Rcomplex(0, 0));

#pragma omp parallel
        {
            // threaded results
            vector<Rcomplex> tmp(t_size_glb, Rcomplex(0, 0));
...
#pragma omp for
            for(int i = 0; i < 2 * lcl_vol; ++i) {
                ...
                WilsonMatrix w = propB[k][i];
                w.hconj();
                WilsonMatrix p[2];

                apply_op_snk(p[0], propA[k][i], snk_op);
...
#pragma omp critical
            for(int t = 0; t < t_size_glb; ++t) {
                meson[t] += tmp[(t+k)%t_size_glb];
            } // critical, for
        }//omp

        // FIXME
        assert(GJP.Snodes() == 1);
        QMP_sum_double_array((double *)meson.data(), 2 * t_size_glb);

        // double norm = 1. / propA.size();
        const double norm = 1;
                    ...
                    const char fn_t[], int traj)
{
    ...
    run_wall_snk(&Awsnk, propA);
    run_wall_snk(&Bwsnk, propB);

    char buf[1024];
    sprintf(buf, fn_t, traj);
    FILE *fp = Fopen(buf, "w");

    for(unsigned src = 0; src < t_size_glb; ++src) {
        if(Awsnk[src].empty()) continue;
        if(Bwsnk[src].empty()) continue;

        vector<Rcomplex> meson(t_size_glb, Rcomplex(0, 0));
        for(unsigned dt = 0; dt < t_size_glb; ++dt) {
            unsigned snk = (src + dt) % t_size_glb;
        ...
        // const double norm = 1. / t_size_glb;
        const double norm = 1.;
        for(unsigned t = 0; t < t_size_glb; ++t) {
            Fprintf(fp, "%3u %3u %17.10e %17.10e\n", src, t,
                    norm * meson[t].real(),
                        ...
                        const char fn_t[], int traj)
{
    ...
    run_wall_snk(&Lwsnk, propL);
    run_wall_snk(&Rwsnk, propR);
    ...
    for(unsigned src = 0; src < t_size_glb; ++src) {
        if(Lwsnk[src].empty()) continue;
        if(Rwsnk[src].empty()) continue;
