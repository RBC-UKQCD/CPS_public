...
{ 
  ...
  if ( argc!=7) { 
    Fprintf(stdout, "Args: doarg-file measarg-file  initial-directory enable_reproduce repro_percent repro_offset\n");
    exit(-1);
  }
  ...
  if ( !do_arg.Decode(argv[1],"do_arg") ) 
    { 
      Fprintf(stdout, "Failedto decode do_arg\n"); exit(-1);
    }
  if ( !meas_arg.Decode(argv[2],"meas_arg"))
    {
      Fprintf(stdout, "Failed to decode meas_arg\n"); exit(-1);
    }
  ...
  Fprintf(stdout,"Logical machine geometry (%d,%d,%d,%d,%d) \n",
  ...
  Fprintf(stdout,"Local lattice volume (%d,%d,%d,%d,%d)\n",
	do_arg.x_node_sites,
	do_arg.y_node_sites,
	do_arg.z_node_sites,
	do_arg.t_node_sites,
  ...
  for(int traj=meas_arg.TrajStart; 
    ...
    lat.GupdCnt(traj); //set the gauge field counter
    ...
    if ( reproduce ) {
      ...
      if ( traj % repro_period == repro_offset ) {
        ...
        Fprintf(stdout, "Reproducing Traj.%d\n",traj);
...
void ReadRNG(MeasArg *meas)
{
  ...
  if ( meas->RNGIO == MeasIOLoad ) { 
    ...
    if ( !LRG.Read(rng_file,meas->IOconcurrency) ) 
      ERR.General(cname,fname,"Failed to read RNG file %s",rng_file);
  }
}
void WriteRNG(MeasArg *meas)
{
  ...
  if ( meas->RNGIO == MeasIOSave ) { 
    ...
    if ( !LRG.Write(rng_file,meas->IOconcurrency) ) 
      ERR.General(cname,fname,"Failed to write RNG file %s",rng_file);
  }
}
...
void checkpoint(MeasArg *meas_arg)
{
  ...
  if ( !meas_arg->Encode(vml_file,"meas_arg")){
    Fprintf(stdout, "bad meas_arg encode\n");
    exit(-1);
  }
  ...
  if ( !do_arg.Encode(vml_file,"do_arg")){
    Fprintf(stdout, "bad do_arg encode\n");
    exit(-1);
  }
...
void run(Lattice &lat, MeasArg &meas_arg) {
  ...
  for(int task = 0; task < meas_arg.TaskList.TaskList_len; task++){
	...
	Fprintf(stdout, "Gauge Fixing the Lattice\n");
	if ( !w_spect_fg_arg.Decode(meas_task.ArgFilename,"w_spect_fg_arg"))
	  {
	    Fprintf(stdout, "Bum w_spect_fg_arg\n"); exit(-1);
	  }
	...
	gauge_fixed = 1;
       ...
       * AlgRandomGauge
       *************************************************/
      if(meas_task.Measurement == MeasAlgRandomGauge){

        Fprintf(stdout, "Random Gauge Fixing the Lattice\n");

	//calculate the random gauge fixing matrices
        AlgRandomGauge rg(lat,&common_arg_fg);
        rg.run();
	
	//apply the random gauge fixing matrices to the lattice links
	AlgRotateGauge rot(lat,&common_arg_fg);
	rot.run();
	...
	Fprintf(stdout,"Measuring hadron masses traj. %d\n",meas_arg.TrajCur);
	int traj = meas_arg.TrajCur;
	...
	if ( !qpropw_arg.Decode(meas_task.ArgFilename,"qpropw_arg"))
	  {
	    Fprintf(stdout, "Bum qpropw_arg\n"); exit(-1);
	  }
  
	if (!gauss_arg.Decode("gauss_arg.vml","gauss_arg"))
	  {
	    Fprintf(stdout, "Failed to decode gauss_arg\n"); exit(-1);
	  }
	CheckDirectory(meas_task.OutputFilestem);
	char qprop_file[256];
