...
#define startcppcomment(where) (where[0] == '/' && where[1] == '/')
...
get_token (token *tokp)
{
  ...
  for (;;)
      ...
      else if (startcomment (where))
	{
      ...
      else if  (startcppcomment(where)) 
	{ 
	  /*Preserve CPP comments as a special token*/
	  is_comment = 1;
	  where +=2;
	  while( *where != '\n' ) where++;
        }
  ...
  /*
   * PAB: Insert a comment token
  if ( is_comment ) { 
    tokp->kind = TOK_COMMENT;
    comment_buf= malloc(strlen(where));
    comment_tmp = comment_buf;
    while( *where ) *(comment_tmp++) = *(where++);
    *(comment_tmp) = *(where);
    tokp->str = comment_buf;
    return;
  }
   */
