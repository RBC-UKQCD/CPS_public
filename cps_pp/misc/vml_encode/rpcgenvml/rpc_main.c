...
struct commandline
  {
    ...
    int pktflag;		/* produce PAB packet code */
    int vmlflag;
  };
...
static void h_output (const char *infile, const char *define,
		      int extend, const char *outfile);
static void pkt_output (const char *infile, const char *define,
		      int extend, const char *outfile);
...
main (int argc, const char *argv[])
{
  ...
  if (cmd.vmlflag) { 
    fprintf(stderr,"Setting VML output\n");
    set_vml();
  }
  ...
  if (cmd.cflag)
    ...
    pkt_output (cmd.infile, "-DRPC_HDR", DONT_EXTEND, cmd.outfile);
  else if (cmd.lflag)
  ...
  else
    {
      ...
      reinitialize ();
      pkt_output (cmd.infile, "-DRPC_HDR", EXTEND, "_pkt.h");
      reinitialize ();
...
add_warning (void)
{
  ...
  fprintf (fout, " * It was generated using PAB's VML system.\n");
  fprintf (fout, " */\n\n");
}
	  ...
	  const char *outfile)
{
  ...
  if (infile && (include = extendfile (infile, ".h")))
    {
      fprintf (fout, "#include <%s%s>\n", auto_inc_pref,include);
      free (include);
      /* .h file already contains rpc/rpc.h */
    }
  else {
      fprintf(fout,"#include <%svml.h>\n",vml_inc_pref);
  }

  if ( in_CPS ) { 
    fprintf(fout,"CPS_START_NAMESPACE\n");
  }
...
/*
 * Outputs a packet class for use with PAB's UDP transporter
 */


static void
pkt_output (const char *infile, const char *define, int extend,
	  const char *outfile)
{
  xdrfunc *xdrfuncp;
  definition *def;
  ...
  fprintf (fout, "#ifndef _PKT_%s\n#define _PKT_%s\n\n", guard,
	   guard);

  incfilename = extendfile (infile, ".h") ;
  f_print(fout,"#include <pkt/rpc_base.h>\n");
  ...
  for (l = defined; l != NULL; l = l->next)
    {
      if ( l->val->def_kind == DEF_PROGRAM ) {
        fprintf(stderr,"QCDOC-RPC Program %s\n",l->val->def_name);
        print_pkt_derivedclass(l->val);
      }
    }
  if (extend && tell == ftell (fout))
    {
