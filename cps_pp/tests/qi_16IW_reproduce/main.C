...
//  chdir (argv[10]);
  ...
  const char *compare_result = argv[11]; //configuration to compare result against
...
//  temp = 0;
  ...
  Matrix *gaugep = lat.GaugeField();
  for(int i=0;i<4*GJP.VolNodeSites();i++){
    gaugecopy[i] = gaugep[i];
  }
  
  bool parallelread = true;
  if(UniqueID()==0) printf("Reading original resultant gauge field\n");
  {
    ...
    readLat.read(lat,compare_result);
    if(UniqueID()==0) printf("Config read.\n");
  }
  
  for(int i=0;i<4*GJP.VolNodeSites();i++){
    for(int j=0;j<18;j++){
      if( fabs(gaugecopy[i].elem(j) - gaugep[i].elem(j) ) > 1e-08 ){
	...
	printf("Reproduction load/save fail (%d,%d,%d,%d; %d): orig %f loaded %f\n",x,y,z,t,mu,gaugecopy[i].elem(j), gaugep[i].elem(j)); exit(-1);
      }
    }
  }
  if(UniqueID()==0) printf("Gauge reproduce successful\n");
