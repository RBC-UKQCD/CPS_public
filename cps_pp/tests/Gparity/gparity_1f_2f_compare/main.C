...
#define RNG_LATT_2F_1F_TEST
  ...
  //we do the same thing in the 1f section below, and also transform the 2f transformed field to 1f, then compare -- they should be equal
  ...
  Matrix *u_gtrans_2f;
  {
    ...
    Matrix *gtrans = (Matrix *) pmalloc(array_size);

    //random gauge transformation 
    //(will not preserve the U U* symmetry as the RNGs for flavour 0 and 1 are different. However this does not matter for the purposes of this test)
    //(if you wanted to do this properly, you should ensure the gauge transformation matrices obey the complex conjugate symmetry)
    for(int flav=0;flav<2;flav++){
      for(int i=0;i<GJP.VolNodeSites();i++){
	LRG.AssignGenerator(i,flav);
	Matrix *m = gtrans+i+flav*GJP.VolNodeSites();
	IFloat * m0 = (IFloat*)m;
	for(int j=0;j<18;j++){
	  *(m0+j) = LRG.Urand();
	}
    ...
    GaugeTransformU(gtrans, *lattice);

    u_gtrans_2f = (Matrix *) pmalloc(array_size);
    memcpy((void*)u_gtrans_2f, (void*)lattice->GaugeField(), array_size); //backup gtrans field

    memcpy((void*)lattice->GaugeField(),(void*)orig_lattice, array_size); //restore gauge field
    LRG = LRGbak; //restore RNGs
    pfree(gtrans);
    pfree(orig_lattice);
  }
#endif

#define RNG_LATT_2F_1F_TEST_2
#ifdef RNG_LATT_2F_1F_TEST_2
  Float *rand_field;
  //in this test we generate a field of random numbers and do the same on the 1f lattice, compare
  {
    LatRanGen LRGbak(LRG);
    int array_size = 2*GJP.VolNodeSites() * sizeof(Float);
    rand_field = (Float *) pmalloc(array_size);
    for(int flav=0;flav<2;flav++){
      for(int i=0;i<GJP.VolNodeSites();i++){
	LRG.AssignGenerator(i,flav);
	rand_field[i+GJP.VolNodeSites()*flav] = LRG.Urand();
      }
    }
    LRG = LRGbak;
  }
...
#define GET(ARGS,A,IDX) ARGS.A.A##_val[IDX]
  ...
  {
    LatRanGen LRGbak(LRG);
    int array_size = doubled_lattice.GsiteSize() * GJP.VolNodeSites() * sizeof(Float);
    Matrix *orig_lattice = (Matrix *) pmalloc(array_size);
    memcpy((void*)orig_lattice, (void*)doubled_lattice.GaugeField(), array_size);
	...
	*(m0+j) = LRG.Urand();
    ...
    GaugeTransformU(gtrans, doubled_lattice);

    //make a 1f version of the saved 2f gauge transformed field
    Matrix * u_gtrans_2f_1fvers = (Matrix *) pmalloc(array_size);
    SingleToDoubleMatrixField dblr(gparity_X,gparity_Y,4,u_gtrans_2f,u_gtrans_2f_1fvers);
    dblr.Run();
	...
	for(int y=0;y<GJP.YnodeSites();y++){
	  for(int x=0;x<GJP.XnodeSites();x++){
	    ...
	    for(int mu=0;mu<4;mu++){
	      int off = doubled_lattice.GsiteOffset(pos) + mu;
	      double* m = (double*)(doubled_lattice.GaugeField()+off);
	      double* mc = (double*)(u_gtrans_2f_1fvers+off);
	      if(fabs(*m - *mc) > 1e-06 || fabs(*(m+1) - *(mc+1)) > 1e-06 ){
		printf("Error: gtrans compare (%d %d %d %d), %d: (%f %f), (%f %f)\n",x,y,z,t,mu,*m,*(m+1),*mc, *(mc+1));
		err=true;
	      }
	    }
	  }
    ...
    if(err){ printf("Failed gauge transform comparison test\n"); exit(-1); }
    if(!UniqueID()) printf("Passed gauge transform comparison test\n");


    memcpy((void*)doubled_lattice.GaugeField(),(void*)orig_lattice, array_size); //restore gauge field
    pfree(orig_lattice);
    pfree(u_gtrans_2f_1fvers);
    pfree(u_gtrans_2f);
    pfree(gtrans);
    LRG = LRGbak; //restore RNGs
  }
