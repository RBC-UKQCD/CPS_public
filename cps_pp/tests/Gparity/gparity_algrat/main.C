...
{ 
  ...
  AlgIntOmelyan ab2(ab1, rat_quo_l, ab2_arg);
  ...
  AlgIntOmelyan ab2_1f(ab1_1f, rat_quo_l_1f, ab2_arg);
  ...
  for(int conf=0; conf< 1; conf ++ ) {
	...
	sprintf(timestamp_file,"timestamp_2f.%d",traj);
	TimeStamp::set_file(timestamp_file);
	TimeStamp::stamp_incr("Starting 2f traj %d",traj);

	if(!UniqueID()){ printf("Starting 2f evolution for traj %d\n",traj); fflush(stdout); }
	AlgHmc hmc(ab2, common_arg_hmc, hmc_arg_pass);
	hmc.run();
	TimeStamp::decr_stamp("Finished 2f traj %d",traj);
	...
	LatticeFactory::Destroy();
      ...
      {
	char timestamp_file[256];
	sprintf(timestamp_file,"timestamp_1f.%d",traj);
	TimeStamp::set_file(timestamp_file);
	TimeStamp::stamp_incr("Starting 1f traj %d",traj);

	if(!UniqueID()){ printf("Starting 1f evolution for traj %d\n",traj); fflush(stdout); }
	AlgHmc hmc(ab2_1f, common_arg_hmc, hmc_arg_pass);
	hmc.run();
	TimeStamp::decr_stamp("Finished 1f traj %d",traj);
	TimeStamp::close_file();
      }
