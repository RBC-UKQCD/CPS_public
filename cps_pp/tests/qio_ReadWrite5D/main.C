...
#error NOT READY: readPropQIO
	...
	#ifdef DO_scrambleProp
	
	for(int ii(0); ii < scSourceSize; ++ii)
	  *(dummy_scSource_read + ii) = 0.0;
	...
	for(int ii(0); ii < fSourcePairsSize; ++ii)
	  *(dummy_fSourcePairs_read + ii) = 0.0;
	...
	for(int ii(0); ii < scSourcePairsSize; ++ii)
	  *(dummy_scSourcePairs_read + ii) = 0.0;
