...
ActionRationalArg rat_heavy;
ActionRationalArg rat_light;
...
{ 
  ...
  if (!rat_heavy.Decode(argv[5],"rat_heavy")){printf("Bum rat_heavy\n"); exit(-1);}
  if (!rat_light.Decode(argv[6],"rat_light")){printf("Bum rat_light\n"); exit(-1);}
  ...
  AlgActionRational heavy(mom, rat_heavy);
  AlgActionRational light(mom, rat_light);

  //!< Construct numerical integrators
  AlgIntAB &ab1 = AlgIntAB::Create(mom, gauge, ab1_arg);
  AlgIntAB &ab2 = AlgIntAB::Create(ab1, heavy, ab2_arg);
  AlgIntAB &ab3 = AlgIntAB::Create(ab2, light, ab3_arg);
...
void checkpoint(int traj)
{
  ...
  sprintf(vml_file,"rat_light.%d",traj);
  if ( !rat_light.Encode(vml_file,"rat_light") ){
    printf("bad rat_light encode\n");
    exit(-1);
  }

  sprintf(vml_file,"rat_heavy.%d",traj);
  if ( !rat_heavy.Encode(vml_file,"rat_heavy") ){
    printf("bad rat_heavy encode\n");
    exit(-1);
  }
