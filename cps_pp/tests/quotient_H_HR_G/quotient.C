...
ActionQuotientArg quo0_arg;
ActionQuotientArg quo1_arg;
...
{ 
  ...
  if ( argc!=12 ) { 
    ...
    printf("\tquo0_arg.vml quo1_arg.vml\n");
    printf("\rat_quo_arg.vml gauge_arg.vml\n");
    printf("\tab1_arg.vml ab2_arg.vml ab3_arg.vml\n");
    printf("\tcurrent_dir \n");
    exit(-1);
  }
  ...
  if ( !quo0_arg.Decode(argv[4],"quo0_arg")){printf("Bum quo0_arg\n"); exit(-1);}
  if ( !quo1_arg.Decode(argv[5],"quo1_arg")){printf("Bum quo1_arg\n"); exit(-1);}
  if ( !rat_quo_arg.Decode(argv[6],"rat_quo_arg")){printf("Bum rat_quo_arg\n"); exit(-1);}
  ...
  AlgActionQuotient quo1(mom, quo1_arg);
  AlgActionRationalQuotient rat_quo(mom, rat_quo_arg);
  AlgActionQuotient quo0(mom, quo0_arg);
  AlgIntSum quo1_rat_quo(quo1, rat_quo, sum_arg);
  
  //!< Construct numerical integrators
  AlgIntAB &ab1 = AlgIntAB::Create(mom, gauge, ab1_arg);
  AlgIntAB &ab2 = AlgIntAB::Create(ab1, quo1_rat_quo, ab2_arg);
  AlgIntAB &ab3 = AlgIntAB::Create(ab2, quo0, ab3_arg);
...
void checkpoint(int traj)
{
  ...
  sprintf(vml_file,"quo0_arg.%d",traj);
  if ( !quo0_arg.Encode(vml_file,"quo0_arg") ){
    printf("bad quo0_arg encode\n");
    exit(-1);
  }

  sprintf(vml_file,"quo1_arg.%d",traj);
  if ( !quo1_arg.Encode(vml_file,"quo1_arg") ){
    printf("bad quo0_arg encode\n");
    exit(-1);
  }
