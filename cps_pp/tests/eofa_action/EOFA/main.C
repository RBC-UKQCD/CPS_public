		...
		for(int i=0; i<evo_arg.measure_w_spect_interval; i++)
    {
			...
			for(int j=0; j<n_step; j++){ 
				...
				VRB.Result(cname, fname, "Result: step[%d]: m_upper=%e m_lower=%e rw_fac=%e norm=%e\n", 
						j, eofa_arg.den_mass.den_mass_val[j], eofa_arg.num_mass.num_mass_val[j], rw_fac[j], norm[j]);
			}
			VRB.Result(cname, fname, "Result: m_upper=%e m_lower=%e rw_fac=%e\n", eofa_arg.num_mass.num_mass_val[0], eofa_arg.den_mass.den_mass_val[n_step-1], rw);
			Fprintf(fp, "%e\t%e\t%0.14e\t%0.14e\n", eofa_arg.num_mass.num_mass_val[0], eofa_arg.den_mass.den_mass_val[n_step-1], rw, norm[0]);
		}
		Fclose(fp);
