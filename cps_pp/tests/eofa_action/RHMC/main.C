...
{ 
  ...
  if(!doext_arg.Decode(CommandLine::arg(), "doext_arg")){ printf("Bum doext_arg\n"); exit(-1); }
	...
	if(!rat_quo_arg.Decode(CommandLine::arg(), "rat_quo_arg")){ printf("Bum rat_quo_arg\n"); exit(-1); }
	...
	for(int conf=0; conf<evo_arg.gauge_configurations; conf ++)
  {
      ...
      if(do_arg.start_conf_kind == START_CONF_FILE){ ReadLatticeParallel(lat, lat_file); }
		...
		AlgActionRationalQuotient s_quark(mom, rat_quo_arg);

		for(int i=0; i<evo_arg.measure_w_spect_interval; i++)
    {
			s_quark.reweight(rw_fac, norm);
			rw = 0.0;
			for(int j=0; j<n_step; j++){ 
				rw += rw_fac[j]; 
				VRB.Result(cname, fname, "Result: step[%d]: m_upper=%e m_lower=%e rw_fac=%e norm=%e\n", 
						j, rat_quo_arg.bsn_mass.bsn_mass_val[j], rat_quo_arg.frm_mass.frm_mass_val[j], rw_fac[j], norm[j]);
			}
			VRB.Result(cname, fname, "Result: m_upper=%e m_lower=%e rw_fac=%e\n", rat_quo_arg.frm_mass.frm_mass_val[0], rat_quo_arg.bsn_mass.bsn_mass_val[n_step-1], rw);
			Fprintf(fp, "%e\t%e\t%0.14e\t%0.14e\n", rat_quo_arg.frm_mass.frm_mass_val[0], rat_quo_arg.bsn_mass.bsn_mass_val[n_step-1], rw, norm[0]);
		}
		Fclose(fp);
